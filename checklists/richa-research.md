Richa — research rundown
	...
	•	Stale-assumption scan — does a DECISIONS.md entry's evidence still hold (e.g. CP-DEC-005 assumes Apliiq has no sandbox; recheck, don't assume).
	•	Vendor/platform change check — Shopify/Apliiq changelog scan for a capability change that could close a native-first gap. Includes consulting Shopify's own agentic assistant directly when a specific capability question needs an authoritative answer — logged here, not left in an unrecorded chat.
	•	Missing-evidence flags — a DECISIONS.md or PRD.md claim whose Evidence field is thin, unsourced, or dated.
	•	Competitor/reference drift — does the benchmark comparison still hold, or has the reference moved.
	•	Distinguishes fact from inference in every finding before it's handed off — per her own role definition.
