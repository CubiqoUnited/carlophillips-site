...
function controlledOrderAuthorizationReady({
  authorization,
  environment,
  releaseRecord,
  now,
  ...
  return Boolean(
    authorization?.schemaVersion ===
      'cp.shopify-controlled-order-authorization.v1' &&
    authorization.status === 'approved' &&
    authorization.owner === 'Product Owner' &&
...
function hasReviewedShopifyAndProviderBinding(releaseRecord) {
  ...
  return Boolean(
    ...
    mappings[0].mappingFingerprint ===
      releaseRecord.physicalSample?.providerMappingFingerprint
  );
}
...
export async function createApprovedHoodieCheckout({
  ...
  if (!exactProductionLaunchAuthorized && releaseRecord.state !== 'released') {
  ...
  const releaseDecision = evaluateProductReleaseEvidence({
    ...
    targetState: exactProductionLaunchAuthorized ? 'staged' : 'released',
  });
  ...
  if (productRead.status !== 'ready' || !cartCapabilityReady) {
...
export async function createControlledMediumCheckout({
  ...
  controlledOrderRequested = process.env.SHOPIFY_CONTROLLED_ORDER_ENABLED ===
    'true',
  storeDomain = process.env.SHOPIFY_STORE_DOMAIN,
  storefrontToken = process.env.SHOPIFY_STOREFRONT_TOKEN,
  checkoutHosts = process.env.SHOPIFY_CHECKOUT_HOSTS,
  ...
  if (
    !controlledOrderAuthorizationReady({
  ...
  if (
    productRead.status !== 'ready' ||
    cartWriteTest.status !== 'evidence_only'
  ) {
    return {
      ok: false,
      reason: 'CONTROLLED_ORDER_CART_CAPABILITY_NOT_VERIFIED',
    };
  }
  ...
  const variant = product.observedVariants?.find((item) =>
    item.selectedOptions?.some(
      (option) =>
        option.name?.toLowerCase() === 'size' &&
        option.value?.toLowerCase() === 'm'
    )
  );
  ...
  if (
    ...
    variant.price?.amount !==
      controlledOrderAuthorization.maximumItemSubtotal.amount ||
    variant.price?.currencyCode !==
      controlledOrderAuthorization.maximumItemSubtotal.currency
  ) {
    return {
      ok: false,
