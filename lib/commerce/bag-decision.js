...
export function resolveBagDecision({ environment, capabilityDecision, cart = null }) {
  ...
  if (capabilityDecision.status !== 'ready') {
    if (canUseFixtureData(environment)) return localPreviewDecision(capabilityDecision.reason);
    return unavailableDecision(environment, capabilityDecision.reason);
  }
