import { canUseFixtureData, getCommerceEnvironment } from '../config/product-visibility';
...
export function assertLocalCartAllowed(environment = getCommerceEnvironment()) {
...
export function unavailableCartError(environment = getCommerceEnvironment(), cause) {
...
export function createEmptyCart(source = 'fixture') {
  return {
...
export function addLocalCartLine(cart, line) {
...
export function updateLocalCartLine(cart, itemKey, quantity) {
...
export function removeLocalCartLine(cart, itemKey) {
  return withCartTotals({
    ...cart,
    items: cart.items.filter(item => item.key !== itemKey).map(item => ({ ...item })),
  });
}

export function withCartTotals(cart) {
...
export async function restoreOrCreateCart({ cartId, getCart, createCart }) {
  if (cartId) {
    try {
...
export function parseCheckoutHosts(value = '') {
  return value
    .split(',')
...
export function validateCheckoutUrl(value, allowedHosts = []) {
  if (!value) return '';
  let url;
  try {
    url = new URL(value);
