...
export function createProductObservation({
  ...
  const { variantFingerprint, observationFingerprint } = observationFingerprints(envelope);
...
export function reviewProductObservation({
  ...
  if (
    ...
    if (
      ...
      blockers.push(blocker(
        ...
        'Review an immutable observation whose canonical contents match both fingerprints.',
  ...
  return {
    ...
    candidateReleasePatch: blockers.length === 0 ? {
      ...
      evidence: reviewApproval.evidence,
    } : null,
  };
}
...
export function attachObservationToProduct(product, observation) {
  return {
    ...
    observation: {
      ...
      observedAt: observation.observedAt,
      observationFingerprint: observation.observationFingerprint,
      reviewStatus: observation.review.status,
    },
  };
}
