...
function observationFactErrors(product, variants) {
  ...
  if (
    ...
    || canonicalJson(Object.keys(product).sort(compareCanonical)) !== canonicalJson(PRODUCT_FACT_KEYS)
  ) {
  ...
  for (const field of ['description', 'vendor', 'productType', 'tagline']) {
    if (typeof product?.[field] !== 'string' || normalizeText(product[field]) !== product[field]) {
      errors.push(`product ${field} invalid`);
    }
  }
  if (
    !Array.isArray(product?.details)
    || canonicalJson(canonicalDetails(product.details)) !== canonicalJson(product.details)
  ) {
  ...
  for (const variant of variants) {
    ...
    if (!Array.isArray(variant?.selectedOptions) || variant.selectedOptions.length === 0) {
      ...
      const optionNames = variant.selectedOptions.map(option => option.name.toLowerCase());
      if (new Set(optionNames).size !== optionNames.length) {
        errors.push('duplicate variant option name');
      }
      const canonicalOptionNames = variant.selectedOptions.map(option => option.name);
      if (expectedOptionNames === null) {
        expectedOptionNames = canonicalOptionNames;
      } else if (canonicalJson(canonicalOptionNames) !== canonicalJson(expectedOptionNames)) {
