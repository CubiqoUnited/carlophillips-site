function messageFor(decision) {
  ...
  if (decision.environment === 'preview' && decision.visibleCount > 0) {
    return decision.commerceAllowed
      ? `${decision.visibleCount} Shopify-backed ${decision.visibleCount === 1 ? 'product is' : 'products are'} active in private staging.`
  ...
  if (decision.environment === 'production' && decision.visibleCount > 0) {
    return decision.commerceAllowed
      ? `${decision.visibleCount} live Shopify ${decision.visibleCount === 1 ? 'product is' : 'products are'} available.`
