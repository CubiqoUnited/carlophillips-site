...
const HOME_HIGHLIGHT_RULES = [
  ['Heavyweight feel', /\bheavyweight\b/i],
  ['Structured fleece', /\bstructured fleece\b/i],
  ['Soft interior', /\bsoft interior\b/i],
  ['CP chest embroidery', /\b(?:cp|chest)[^.!]{0,24}embroider/i],
];
...
function toHomeHighlights(product) {
  const reviewedCopy = [product.description, ...(product.details || [])]
    .filter(Boolean)
    .join(' ');
  return HOME_HIGHLIGHT_RULES
    .filter(([, pattern]) => pattern.test(reviewedCopy))
    .map(([label]) => label);
}
...
export function toHomeCatalogSummary(decision) {
  ...
  return {
    ...
    primaryProduct: first ? {
      ...
      highlights: toHomeHighlights(first),
      heroMedia,
      media,
    } : null,
  };
