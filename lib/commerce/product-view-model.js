import {
  toProviderNeutralMediaAlt,
  toProviderNeutralMediaLabel,
...
function normalizeMediaItem(item, index, title) {
  ...
  return {
    ...
    label: toProviderNeutralMediaLabel(item.label || type.replaceAll('_', ' ')),
  };
}
...
export function toProductViewModel(decision) {
  ...
  const localColors =
    product.variants?.colors || (product.color ? [product.color] : []);
  ...
  return {
    ...
    media: (product.media || []).map((item, index) =>
