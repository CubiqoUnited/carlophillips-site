...
function releasePresentation(decision) {
  ...
  if (
    ...
    && decision.reason === 'SINGLE_PRODUCT_COMMERCE_LAUNCH_APPROVED'
  ) {
    return {
      sourceLabel: 'Shopify Storefront — live product',
      truthHeading: 'Current Shopify product facts.',
      story: 'The Signature Hoodie is the first live CARLOPHILLIPS product. Richer approved product media will be added as it is produced and verified.',
  ...
  if (decision.source === 'shopify' && decision.environment === 'local') {
    return {
      sourceLabel: 'Shopify Storefront observation — local review',
