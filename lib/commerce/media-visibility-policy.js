...
function isApprovedAsset(asset) {
  return asset?.exactProductMatch === 'verified'
...
export function fingerprintStorefrontMedia(item) {
  ...
  return hashStorefrontMediaReference(JSON.stringify({
    id: item.id.trim(),
    type: item.type.trim(),
    url,
    previewUrl,
...
export function filterReleaseBoundMedia({ product, manifest }) {
  ...
  const manifestAssetsById = new Map(assets.map(asset => [asset?.assetId, asset]));
  const approvedKindsByAssetId = new Map();
  const modalitiesByAssetId = new Map();
  for (const requirement of requirements.filter(item => item.status === 'approved')) {
    for (const assetId of requirement.assetIds) {
      const kinds = approvedKindsByAssetId.get(assetId) || new Set();
      for (const kind of MODALITY_KINDS[requirement.modality] || []) kinds.add(kind);
  ...
  for (const asset of assets) {
    ...
    if (assetsByBinding.has(referenceHash)) duplicateBindings.add(referenceHash);
    assetsByBinding.set(referenceHash, asset);
  }
  for (const referenceHash of duplicateBindings) assetsByBinding.delete(referenceHash);
  ...
  for (const item of observedMedia) {
    ...
    media.push({
      ...
      alt: asset.alt,
      label: [...(modalitiesByAssetId.get(asset.assetId) || ['accessible fallback'])]
        .join(' / '),
    });
  }
  ...
  const missingFallbackAssetIds = media
    ...
    .filter(asset => ['video', 'external_video', 'model_3d', 'spin'].includes(asset?.kind))
  ...
  const coveredModalities = requirements
    ...
    .map(requirement => requirement.modality);
  const productionReady = missingModalities.length === 0 && missingFallbackAssetIds.length === 0;
  ...
  return {
    product: {
      ...
      media,
      heroImage: media.find(item => item.previewUrl)?.previewUrl
