...
import { GET_PRODUCT_BY_HANDLE } from '../../shopify/queries.js';
import { normalizePublicShopifyProduct } from './public-product-json-adapter.js';
import {
  attachObservationToProduct,
  createProductObservation,
} from '../../commerce/product-observation.js';
...
export function createShopifyProductLoader({
  ...
  if (storeDomain === '' || storefrontToken === '') {
    const error = new Error('Shopify Storefront configuration is incomplete');
    error.code = 'SHOPIFY_NOT_CONFIGURED';
    throw error;
  }

  const normalizedDomain = (storeDomain || 'carlophillips.myshopify.com').replace(/^https?:\/\//, '').replace(/\/$/, '');
  ...
  return async function loadProduct(handle) {
    ...
    // Public Shopify Product JSON fallback (no credentials needed)
    const sourceUrl = `https://${normalizedDomain}/products/${handle}.js`;
    const response = await fetchImpl(sourceUrl, { method: 'GET', cache: 'no-store' });
    if (!response.ok) {
      const error = new Error(`Shopify public product JSON returned HTTP ${response.status}`);
      error.code = 'SHOPIFY_HTTP_ERROR';
      throw error;
    }
    const rawJson = await response.json();
    const product = normalizePublicShopifyProduct(rawJson, { currency: 'USD' });
    if (!product) return null;
