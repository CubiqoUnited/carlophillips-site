...
import { createPipelineRun } from './pipeline-run.js';
...
export function computeCreationInputFingerprint(job) {
  const {
    briefId: _briefId,
    ...briefInputs
  } = job.brief;
...
function assertFreshness(evidence) {
  const publishedAt = evidence.publishedAt === null ? null : Date.parse(evidence.publishedAt);
  ...
  if (
    ...
    || (publishedAt !== null && (!Number.isFinite(publishedAt) || publishedAt > observedAt))
    || retrievedAt < observedAt
    || evaluatedAt < retrievedAt
  ) {
    throw new Error(`Evidence ${evidence.evidenceId} has an invalid freshness window.`);
  }

  const { maxAgeHours, status } = evidence.freshness;
  if (status === 'not-time-sensitive') {
    if (maxAgeHours !== null) {
      throw new Error(`Evidence ${evidence.evidenceId} must use a null age limit when not time-sensitive.`);
    }
    return;
  }
  if (!Number.isInteger(maxAgeHours) || maxAgeHours < 1) {
    throw new Error(`Evidence ${evidence.evidenceId} requires a positive freshness limit.`);
  }

  const ageHours = (evaluatedAt - observedAt) / 3_600_000;
  if (status === 'current' && ageHours > maxAgeHours) {
    throw new Error(`Evidence ${evidence.evidenceId} is labeled current beyond its freshness limit.`);
  }
  if (status === 'stale' && ageHours <= maxAgeHours) {
    throw new Error(`Evidence ${evidence.evidenceId} is labeled stale inside its freshness limit.`);
  }
}
...
function assertInputPolicy(environment, inputEvidence) {
  for (const evidence of inputEvidence) {
    if (!evidence.provenance?.publisher || !evidence.provenance?.evidenceRef) {
      throw new Error(`Evidence ${evidence.evidenceId} requires explicit provenance.`);
    }
    ...
    if (
      ...
      throw new Error('Trend signals require a time-bounded current or stale classification.');
    }
...
function assertTriggerPolicy(trigger) {
  ...
  if (
    ...
    throw new Error('Scheduled creation jobs require an explicit cadence, expression, and timezone.');
  }
}
...
function assertFailClosedJob(job) {
  ...
  if (
    ...
    throw new Error('Creation jobs require binding brand rules and prohibited claims.');
  }
  if (Object.values({
    mayCopy: job.brief.referenceUsePolicy?.mayCopy,
    mayAssertRights: job.brief.referenceUsePolicy?.mayAssertRights,
    mayAssertProductTruth: job.brief.referenceUsePolicy?.mayAssertProductTruth,
    mayAssertMediaTruth: job.brief.referenceUsePolicy?.mayAssertMediaTruth,
  }).some(value => value !== false) || job.brief.referenceUsePolicy?.mode !== 'inspiration-only') {
    throw new Error('References are inspiration-only and cannot grant copying, rights, product, or media truth.');
  }
  if (
    ...
    throw new Error('Research must remain attributed, freshness-labeled, human-pending, and externally gated.');
  }
  ...
  if (
    ...
    throw new Error('Creation job idempotency fingerprint is missing or inconsistent.');
  }
}
...
export function registerCreationJob(existingJobs, job) {
  assertFailClosedJob(job);
  const duplicate = existingJobs.find(existing => (
