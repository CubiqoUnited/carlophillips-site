...
function validateUnsigned(receipt) {
  ...
  if (receipt.schemaVersion !== 'cp.protected-staging-release-receipt.v2'
  ...
  for (const variant of shopify.variants) {
    exactKeys(variant, [
      'size', 'price', 'currency', 'availableAtSnapshot', 'availableAtProof',
      'inventoryAtSnapshot', 'inventoryAtProof', 'inventoryUnchanged',
    ], 'SHOPIFY_VARIANTS_INVALID');
    sizes.push(variant.size);
    if (variant.price !== '128.00' || variant.currency !== 'USD'
      || !Number.isSafeInteger(variant.inventoryAtSnapshot)
      || !Number.isSafeInteger(variant.inventoryAtProof)
      || variant.inventoryAtSnapshot < 1
      || variant.inventoryAtProof !== variant.inventoryAtSnapshot) {
      fail('SHOPIFY_VARIANTS_INVALID');
    }
    truthy(variant.availableAtSnapshot, 'SHOPIFY_VARIANTS_INVALID');
    truthy(variant.availableAtProof, 'SHOPIFY_VARIANTS_INVALID');
    truthy(variant.inventoryUnchanged, 'SHOPIFY_INVENTORY_MUTATED_DURING_PROOF');
  }
