...
function validateUnsigned(receipt) {
  ...
  if (receipt.schemaVersion !== 'cp.protected-staging-release-receipt.v1'
  ...
  for (const variant of shopify.variants) {
    exactKeys(variant, [
      'size', 'price', 'currency', 'availableBefore', 'availableAfter',
      'inventoryBefore', 'inventoryAfter', 'inventoryRestored',
    ], 'SHOPIFY_VARIANTS_INVALID');
    sizes.push(variant.size);
    if (variant.price !== '128.00' || variant.currency !== 'USD'
      || !Number.isSafeInteger(variant.inventoryBefore)
      || !Number.isSafeInteger(variant.inventoryAfter)
      || variant.inventoryBefore < 1
      || variant.inventoryAfter !== variant.inventoryBefore) {
      fail('SHOPIFY_VARIANTS_INVALID');
    }
    truthy(variant.availableBefore, 'SHOPIFY_VARIANTS_INVALID');
    truthy(variant.availableAfter, 'SHOPIFY_VARIANTS_INVALID');
    truthy(variant.inventoryRestored, 'INVENTORY_NOT_RESTORED');
  }
  ...
  exactKeys(receipt.customerPath, [
    ...
    'hostedStagingCheckout', 'testPayment', 'paidOrder',
    'brandedConfirmation', 'confirmationEvidenceHash', 'orderStatus',
    'orderStatusEvidenceHash',
  ], 'CUSTOMER_PATH_INCOMPLETE');
  for (const key of [
    'shopifyAuthoritativeProduct', 'sizeSelection', 'bagTruth',
    'hostedStagingCheckout', 'testPayment', 'paidOrder',
    'brandedConfirmation', 'orderStatus',
  ]) {
    const result = receipt.customerPath[key];
    truthy(result, 'CUSTOMER_PATH_INCOMPLETE');
  }
  if (!FINGERPRINT.test(receipt.customerPath.confirmationEvidenceHash || '')
    || !FINGERPRINT.test(receipt.customerPath.orderStatusEvidenceHash || '')) {
    fail('CUSTOMER_PATH_EVIDENCE_HASH_INVALID');
  }

  const lifecycle = receipt.lifecycle;
  exactKeys(lifecycle, [
    'orderReferenceHash', 'signatureVerified', 'signatureAlgorithm',
    'durableIdempotency', 'requiredTopics', 'observedTopics',
    'duplicateDelivery', 'cancelled', 'refunded', 'inventoryRestored',
    'finalFinancialStatus', 'finalFulfillmentStatus',
    'fulfillmentRequestSubmitted', 'apliiqLocationAssigned',
    'noApliiqProductionJob',
  ], 'LIFECYCLE_EVIDENCE_INVALID');
  if (!FINGERPRINT.test(lifecycle.orderReferenceHash || '')
    || lifecycle.signatureAlgorithm !== 'shopify-hmac-sha256'
    || stable([...lifecycle.requiredTopics].sort()) !== stable([...REQUIRED_TOPICS].sort())) {
    fail('LIFECYCLE_EVIDENCE_INVALID');
  }
  for (const topic of REQUIRED_TOPICS) {
    if (!lifecycle.observedTopics.includes(topic)) fail('LIFECYCLE_TOPICS_INCOMPLETE');
  }
  truthy(lifecycle.signatureVerified, 'LIFECYCLE_SIGNATURE_UNVERIFIED');
  truthy(lifecycle.durableIdempotency, 'LIFECYCLE_IDEMPOTENCY_UNVERIFIED');
  exactKeys(
    lifecycle.duplicateDelivery,
    ['attempted', 'suppressed', 'observationCount', 'externalActionCount'],
    'DUPLICATE_PROOF_INVALID'
  );
  truthy(lifecycle.duplicateDelivery.attempted, 'DUPLICATE_PROOF_INVALID');
  truthy(lifecycle.duplicateDelivery.suppressed, 'DUPLICATE_PROOF_INVALID');
  if (
    lifecycle.duplicateDelivery.observationCount !== 1
    || lifecycle.duplicateDelivery.externalActionCount !== 0
  ) fail('DUPLICATE_ACTION_DETECTED');
  truthy(lifecycle.cancelled, 'CANCELLATION_UNVERIFIED');
  truthy(lifecycle.refunded, 'REFUND_UNVERIFIED');
  truthy(lifecycle.inventoryRestored, 'INVENTORY_NOT_RESTORED');
  truthy(lifecycle.noApliiqProductionJob, 'APLIIQ_JOB_DETECTED');
  if (lifecycle.finalFinancialStatus !== 'REFUNDED'
    || lifecycle.finalFulfillmentStatus !== 'UNFULFILLED') {
    fail('FINAL_ORDER_STATE_INVALID');
  }
  if (lifecycle.fulfillmentRequestSubmitted !== false
    || lifecycle.apliiqLocationAssigned !== false) {
    fail('APLIIQ_JOB_DETECTED');
  }
  ...
  exactKeys(receipt.safeguards, [
    'piiFree', 'realPaymentUsed', 'productionOrderCreated',
...
export function verifyProtectedReleaseReceipt(receipt, expected, secret) {
  exactKeys(receipt, [
    ...
    'sourcePullRequest', 'staging', 'shopify', 'customerPath', 'lifecycle',
...
export const PROTECTED_RELEASE_REQUIRED_TOPICS = REQUIRED_TOPICS;
