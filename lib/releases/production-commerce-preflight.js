...
export function evaluateProductionCommercePreflight({
  ...
  const candidateSha = releaseRecord?.candidate?.gitCommit;
  if (sourceRelationship && sourceRelationship.ready !== true) {
    blockers.push(blocker(
      'SOURCE_RELATIONSHIP_INVALID',
      `Git source verification failed: ${(sourceRelationship.blockers || []).join(', ') || 'unknown relationship failure'}.`,
    ));
  }
  if (candidateSha !== expectedSha && !(
    sourceRelationship?.ready === true
    && sourceRelationship?.mode === 'evidence_only_descendant'
    && sourceRelationship?.candidateSha === candidateSha
    && sourceRelationship?.expectedSha === expectedSha
  )) {
    blockers.push(blocker(
      'RELEASE_CANDIDATE_SOURCE_NOT_EQUIVALENT',
      'The selected main SHA must be the reviewed candidate or a Git-verified evidence-only descendant.',
    ));
  }
  ...
  return {
    ...
    sourceMode: candidateSha === expectedSha
      ? 'exact_candidate'
      : (sourceRelationship?.ready ? sourceRelationship.mode : 'denied'),
