...
export function deriveAdminControlPlane({ release, mediaManifest, run, registry, authorities, readiness }) {
  ...
  return {
    ...
    lifecycle: {
      orders: {
        status: 'missing',
        title: 'No controlled order exists.',
        detail: 'No release-bound payment, order, POD handoff, fulfillment, shipment, delivery, or reconciliation event has been ingested.',
      },
      postSale: {
        status: 'missing',
        title: 'No post-sale case exists.',
        detail: 'Support, return, refund, and review eligibility have no selected owner, controlled-order record, or exercised lifecycle.',
      },
      analytics: {
        status: 'blocked',
        title: 'No approved analytics ledger exists.',
        detail: 'Provider, consent-aware event dictionary, retention, access, and Shopify reconciliation are not approved or implemented.',
      },
    },
  };
}
