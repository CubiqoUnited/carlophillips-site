...
export function deriveMediaGenerationWorkspace({ workspace, release, mediaManifest, featureEnabled, role, providerHandshakes = [] }) {
  ...
  return {
    ...
    metrics: {
      ...
      storefrontBound: workspace.candidates.filter(candidate => candidate.storefrontBound).length,
      stagingPreviewBound: workspace.candidates.filter(candidate => candidate.stagingPreviewBound).length,
    },
    candidateCounts,
    actions,
    boundary: 'Feature-flagged Staging review workspace. Product Owner-approved preview media may be presented on Staging, while the canonical Product Release Record, Shopify publication, payment authority, and Production remain unchanged.',
  };
}
