...
| Admin control plane | Local-only overview and 12 operational sections derive from canonical artifacts; a four-value Theme screen uses a distinct Product Owner credential and local branch-only repository proposal | No remote identity/RBAC, durable read model, append-only persistence, GitHub automation, or external commands; Theme can write only `theme.json` and never Production | Approve identity, database/event store, users/roles, privacy/retention and incident ownership before remote access; keep Theme in the normal PR/Preview/review/merge path and introduce reviewed idempotent commands one connector at a time |
