...
| Admin control plane | Local-only Overview, Evidence Health, Commands, and 12 other domain sections derive from canonical artifacts; a four-value Theme screen uses a distinct Product Owner credential and local branch-only repository proposal; a pure command policy proves exact fail-closed authorization decisions | Commands truthfully shows an empty queue and unavailable execution dependencies; policy denial/authorization fixtures are locally proven, but no remote identity/RBAC, durable idempotency/audit/read model, connector decision, executor, GitHub automation, or external command exists; Theme can write only `theme.json` and never Production | Approve identity, database/event store, users/roles/grants, privacy/retention and incident ownership before remote access; keep Theme in the normal PR/Preview/review/merge path, then wire reviewed idempotent commands through durable audit and one authorized connector at a time |
