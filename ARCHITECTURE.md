...
The candidate has two deliberately separate presentation surfaces in one repository. Public customer routes remain release-derived. `/admin` is a local-only, bearer-gated operational projection over sanitized committed artifacts. It has no public navigation, no raw Shopify/POD references, and no commerce or release authority. Evidence Health separates technical access, freshness, source history, and operating authority; the July Shopify-login blocker is superseded history while its later August evidence is itself stale and non-authorizing. Every operational area remains read-only except a Product Owner-only Theme screen. Theme can atomically replace only the validated root `theme.json` on a local `codex/*` branch with optimistic revision protection; it cannot change layout, create a commit/PR/Preview, deploy, publish, or touch Production. Every Vercel environment and every non-local commerce environment is hard-denied until an approved identity provider, RBAC, durable event/read-model store, retention policy, and incident owner exist.
...
| Admin control plane | Local-only Overview, Evidence Health, and 12 domain sections derive from canonical artifacts; a four-value Theme screen uses a distinct Product Owner credential and local branch-only repository proposal | No remote identity/RBAC, durable read model, append-only persistence, GitHub automation, or external commands; Theme can write only `theme.json` and never Production | Approve identity, database/event store, users/roles, privacy/retention and incident ownership before remote access; keep Theme in the normal PR/Preview/review/merge path and introduce reviewed idempotent commands one connector at a time |
| Order/post-sale | PII-safe release/variant/environment-bound event schema, pure strict lifecycle reducer, hash chain, idempotency/replay guards, authority checks, and sanitized admin projection exist locally | Happy/exception transitions are proven locally; no webhook ingress, durable storage, provider/customer data, controlled order, payment, POD fulfillment, shipment, tracking, support, return/refund, review, or reconciliation observation exists | Select one owner/adapter per capability, add approved signed ingress and durable inbox/outbox, then exercise exactly one approved order through every terminal state before claiming readiness |
