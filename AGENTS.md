Class LOCKED  ·  Owner Boss  ·  Writers Boss only — roles append proposals to PROPOSALS.md  ·  Read standing — every role  ·  Cadence rarely  ·  Budget 200–250 lines (now 443)
CP project rules
	•	Cold start is exact: state/NOW.md → state/BLOCKERS.md → state/SCOPE.md → highest-priority READY item in docs/exec-plans/active/phase-1.md → invoked agents/<role>.md → matching checklist. Do not preload other context.
	•	If no READY item is owned by the invoked role, exit silently: no file, rundown, session record, or message. Close a session only when material project truth changed.
	•	Sushma is the sole writer and reconciler of state/NOW.md. Other roles never edit it; they write a collision-proof immutable rundown/session record, then hand the record to Sushma for reconciliation.
	•	New records use state/rundowns/YYYY-MM-DDTHHMMSSZ-<role>-<slug>.md or state/sessions/YYYY-MM-DDTHHMMSSZ-<role>-<slug>.md. Once closed they are immutable. Required handoff fields are defined in each directory's SCHEMA.md.
	•	A null cost_limit means $0 and forbids paid actions. Industry estimates may forecast cost only; they never authorize spend. Boss alone approves monetary limits or paid actions.
	...
	•	Scope discipline: a role identifying work outside SCOPE.md's Included list appends a collision-safe, role-owned entry to state/PROPOSALS.md. The role does not build first and explain later.
	...
	•	Any role's recommended governed-file change is appended to state/PROPOSALS.md under a collision-safe entry ID owned by that role. Existing entries are append-only and never rewritten by another role; proposals are never left chat-only.
