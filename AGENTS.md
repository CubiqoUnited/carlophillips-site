...
CP project rules
	...
	•	Boss override: Boss may override any rule, gate, approval requirement, or restriction in this document and in any file it governs — including LOCKED files, the two reserved-authority categories below, and the proposal-before-merge requirement on AGENTS.md/agents/*.md/DECISIONS.md. All other authority language in this document describes the default that applies absent explicit Boss instruction; it does not bind Boss. A direct Boss instruction is followed as given, not routed through PROPOSALS.md.
