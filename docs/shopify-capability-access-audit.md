...
|---|---|---|---|---|
| P0 | Shopify Storefront product/variant/media read | Server adapter and canonical observation contract exist; capability evidence/config remain unverified | Authorized owner configures least-privilege read-only values in an ignored local or Preview environment and records the durable capability evidence reference | Run the product-by-handle observation, review the exact sanitized envelope fingerprint/handle, and keep the resulting candidate patch unapplied until separately authorized |
