...
- The release branch is published for review in PR #8. Production remains unchanged on the approved `bb9568f` visual baseline; the corrected candidate must receive a fresh immutable Preview and independent parity GO before the Product Owner merges it to canonical `main` and the exact reviewed artifact can be promoted. No Shopify write, catalog/order mutation, domain change, or billing action occurred.
