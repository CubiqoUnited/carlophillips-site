...
- PR #8 merged the first v1.2.2 candidate to canonical `main`; its independent parity review remained NO-GO and it was not promoted to Production. The corrected follow-up is published for review in PR #9 from immutable commit `cfef2e3`. Production remains unchanged on the approved `bb9568f` visual baseline until the corrected Preview receives independent GO, PR #9 reaches canonical `main`, and the exact reviewed artifact is promoted. No Shopify write, catalog/order mutation, domain change, or billing action occurred.
