...
const allowedModalities = new Set<string>(PRODUCT_MEDIA_MODALITIES);
const allowedOnBodyPoses = new Set<string>(ON_BODY_POSES);
const allowedConstructionDetails = new Set<string>(CONSTRUCTION_DETAILS);
const allowedMotionRoles = new Set<string>(['film', 'preview']);
...
function isModalityArray(value: unknown): value is ProductMediaModality[] {
  return (
    ...
    value.length > 0 &&
    value.every(
      (item) => typeof item === 'string' && allowedModalities.has(item)
    )
  );
}
...
export function mapEligibleMediaItem(
  ...
  return {
    ...
    ...(hasText(item.onBodyPose) && allowedOnBodyPoses.has(item.onBodyPose)
      ? { onBodyPose: item.onBodyPose as OnBodyPose }
      : {}),
    ...(hasText(item.constructionDetail) &&
    allowedConstructionDetails.has(item.constructionDetail)
      ? { constructionDetail: item.constructionDetail as ConstructionDetail }
      : {}),
    ...(hasText(item.motionRole) && allowedMotionRoles.has(item.motionRole)
      ? { motionRole: item.motionRole as 'film' | 'preview' }
      : {}),
  };
}
