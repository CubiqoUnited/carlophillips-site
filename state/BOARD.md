...
- **`CP-VARIANT-001` is factually wrong.** It records *6 variants, DENY, tracked, availableForSale false*. The production store reads **3 variants, `CONTINUE`, untracked, `availableForSale: true`**. The row describes a state that does not exist. Superseded by **KAN-6 → closed**, replaced by Pushpa's **AC-OVS-1**: no surface may assert scarcity, and "sold out" must never derive from an inventory count.
- **`CP-GATEWAY-001` closes on D-019** for the production leg — direct read of the Test-mode toggle, OFF. Only the staging leg survives, and it is not a formality: #1005 was a test payment that produced a real dispatch.
- **Rows citing H-005 as a live blocker are stale.** H-005 is RESOLVED; **H-007** replaces it — staging is not reachable programmatically, so its reads are manual.
- **`CP-DEF2-001` / KAN-5**: fix exists, verified NOT on `github/main`. Ship problem, not a code problem. Blast radius smaller than graded — no wrong store, no wrong webhook secret.
- **The Rapid Logo Tee is ARCHIVED** (Sushma, Boss's instruction). KAN-18 and KAN-10 close with it.
- **Vercel invoice P0 withdrawn** on Boss's instruction. **No P0 remains.**
- **Jira is now the canonical work record** under Boss mandate M-003. The KAN project, not this table, is where open items live; the open-items register is a comment on **KAN-2**. This board remains the delivery-state projection, not the issue list.
...
| CP-CAT-001 | Catalogue | P1 LAUNCH — **dominant blocker** | READY | Boss/Pushpa — sourcing + content, not engineering | none — unblocked, spec supplies the target | UI-SPEC: 6 categories × 6 items = 36; live = 1 category, 1 product. Min credible launch: Pushpa recommends Option B, 6 Hoodies + 6 T-shirts, zero code change (work/items/CP-CAT-001-product-template.md §4) | 2026-09-18T00:00:00Z | 5–10 items per category | READY_FOR_PO | Boss | FRESH |
| ADR-0001 | Catalogue sizing/nav | P1 LAUNCH | APPROVED — ready to build | Aarti | none — A3 code-identity closed (§13); pre-existing deployment-ID drift (NOW.md) is separate and does not gate this build | Pushpa APPROVED §12 (071500Z); A2 closed; A3 §13 — cited files byte-identical between local HEAD and github/main+staging, no divergence | 2026-09-19T00:00:00Z | Build against the pushed tree | IN_BUILD | Aarti | FRESH |
