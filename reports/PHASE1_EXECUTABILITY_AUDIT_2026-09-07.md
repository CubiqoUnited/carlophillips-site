...
| Cancellation, return, refund, replacement and post-production exception rules | Pushpa rules accepted in `CP-DEC-004`; ten-scenario tabletop complete on PR #119; reconciled exact head passes 55/55 locally and exact-head CI | Already complete for rules/tabletop; live proof blocked by another human gate | Non-live response preparation is closed. Native execution still depends on protected Staging access/config and, where applicable, the controlled order | After access clears, Aarti runs native Staging drills and Pushpa records operational business acceptance |
...
6. The non-live exception tabletop was the only newly identified safe tranche
   executable without one of those gates. It was resumed and completed on PR
   #119 and reconciled onto PR #118's accepted head. Exact head
   `1deed62861acaf5aaed9b32743210033f3339725` passes 55/55 supporting
   synthetic tests locally and exact-head GitHub `Verify` passes.
