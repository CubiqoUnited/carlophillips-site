...
|---|---|---|---|---|---|---|---|---|---|---|---|
| HI-P1-STAGING-VERCEL | ADMIN / EXTERNAL ACCESS REQUIRED | P1 | Protected Staging deployment for `staging@ce2bb181…`, which gates subsequent tranche merges | Reauthorize or replace GitHub Staging secret `VERCEL_TOKEN` with a token that can access Vercel scope `cubiqo-projects-d7156840`; do not expose the value | GitHub/Vercel admin owner; Aarti verifies; Sushma reruns/closes | Corrected protected run `34184480979` passed exact-SHA/repository verification, then `vercel pull` returned `scope-not-accessible` before build/deploy/alias. Available authority does not permit fabricating or copying credentials | PR #117 exact-head checks and non-live preparation continue; #118/#119 stay ordered | Protected workflow identity can access the configured Cubiqo org/project | Rerun `vercel-staging.yml --ref staging` with PR `116`, expected SHA `ce2bb181d9a62e5b62af9086c4085b60163077e2`, release `phase1-governance-ce2bb181`; verify receipt and alias before merging #117 | Canonical Staging verification and all later tranche merges; not local/synthetic preparation | OPEN |
