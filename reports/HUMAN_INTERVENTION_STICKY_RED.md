...
|---|---|---|---|---|---|---|---|---|---|---|---|
| HI-GITHUB-STAGING-RULESET | ADMIN / EXTERNAL ACCESS REQUIRED | P1 | Delegated Staging merge authority | A repository administrator must first create an active `Protect staging delivery` branch ruleset targeting only `staging`, with pull requests required, zero approving reviews, resolved conversations, up-to-date branches, required `Verify` and `Checkout E2E and accessibility` checks, and deletion/force-push blocked; then edit `Protect main and staging` to target only the default branch while preserving all existing `main` rules and approvals | GitHub repository administrator; Sushma verifies | CLI identity `avloy07-eng` has `WRITE`, not `ADMIN`; the signed-in `CubiqoUnited` browser session returned `Unauthorized` when the prepared ruleset was submitted | Phase 1 implementation, automated checks, role PASS/APPROVE evidence, and other non-conflicting work continue; existing human approvals remain required until the live rule changes | GitHub API shows two active rulesets: protected `main` with existing approval requirements, and protected `staging` with PR + required checks + zero human approvals | Recheck open PR mergeability, preserve Boss canonical-Staging validation before Production, and close/supersede staging-only independent-approval queue items | Staging merge governance only; not all Phase 1 work | OPEN |
| HI-116-APPROVAL | INDEPENDENT GITHUB APPROVAL | P1 | Governance activation | Independently review and approve the latest head of PR #116 after its required checks pass | Eligible independent GitHub reviewer; Sushma prepares the approval brief and tracks | The author cannot self-approve and repository governance requires one approval of the latest push | Next-tranche architecture/preparation, business-rule completion, and non-conflicting synthetic/read-only work | GitHub records approval of the latest head and PR #116 becomes mergeable | `merge → protected Staging verification → protected Production promotion/verification` | Governance task, Production activation; not all Phase 1 work | READY FOR REVIEW |
| HI-117-APPROVAL | INDEPENDENT GITHUB APPROVAL | P1 | Support/customer-account tranche | Independently review and approve PR #117 latest head `ee71bbd` after required exact-head checks pass | Eligible independent GitHub reviewer; Sushma prepares the approval brief and tracks | The author cannot self-approve; repository governance requires latest-head checks and independent approval. PR #116 is not a technical review dependency | Live configuration work and other non-conflicting Phase 1 reliability preparation | Required checks pass and latest-head approval is recorded; reconcile with canonical `staging` before protected merge if its base moved | `merge #117 → protected Staging deploy → support/account/returns operational drills` | One tranche and Phase exit | READY FOR REVIEW |
| HI-118-APPROVAL | INDEPENDENT GITHUB APPROVAL | P1 | Production Watch and webhook recovery | Independently review and approve PR #118 latest head `fb4bdfe` after required exact-head checks pass | Eligible independent GitHub reviewer; Sushma prepares the approval brief and tracks | The author cannot self-approve; scheduled monitoring and webhook recovery must enter through protected review. PR #116 is not a technical review dependency | Controlled-order preparation and synthetic exception drills | Required checks pass and latest-head approval is recorded; reconcile with canonical `staging` before protected merge if its base moved | `merge #118 → protected Staging validation → normal Production promotion → observe first scheduled watch` | Monitoring tranche, Phase exit, Production activation | READY FOR REVIEW |
| HI-119-APPROVAL | INDEPENDENT GITHUB APPROVAL | P1 | Controlled lifecycle runbook and synthetic drills | Review PR #119 after PR #118; approve only after it is retargeted/reconciled through canonical `staging` and latest-head checks pass | Eligible independent GitHub reviewer; Sushma prepares the approval brief and tracks | The author cannot independently approve the operating evidence contract | Ten-scenario tabletop is reconciled onto #118; 55/55 local synthetic tests and exact-head CI pass at `1deed62`; no transaction is allowed | PR #118 is merged, PR #119 is retargeted to `staging`, required checks pass, and latest-head approval is recorded | `merge #119 → run exact-SHA workflow → retain explicitly synthetic receipt → prepare live authority gate` | Controlled-proof preparation and Phase exit; not unrelated Phase work | READY AFTER #118 |
