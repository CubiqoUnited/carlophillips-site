...
| HI-117-APPROVAL | P1 | Support/customer-account tranche | Independently review and approve the latest head of PR #117 after PR #116 is merged and the branch is reconciled with canonical `staging` | Requested reviewer: `CubiqoUnited`; Sushma tracks | The author cannot self-approve; the protected merge requires latest-base checks and independent approval | Live configuration work and other non-conflicting Phase 1 reliability preparation | PR #116 is verified on Staging, PR #117 is rebased, required checks pass, and latest-head approval is recorded | `merge #117 → protected Staging deploy → support/account/returns operational drills` | One tranche and Phase exit | OPEN |
| HI-118-APPROVAL | P1 | Production Watch and webhook recovery | Independently review and approve the latest head of PR #118 after PR #116 is merged and the branch is reconciled with canonical `staging` | Requested reviewer: `CubiqoUnited`; Sushma tracks | The author cannot self-approve; scheduled monitoring and webhook recovery must enter through protected review | Controlled-order preparation and synthetic exception drills | PR #116 is verified on Staging, PR #118 is rebased, required checks pass, and latest-head approval is recorded | `merge #118 → protected Staging validation → normal Production promotion → observe first scheduled watch` | Monitoring tranche, Phase exit, Production activation | OPEN |
| HI-119-APPROVAL | P1 | Controlled lifecycle runbook and synthetic drills | Review the latest stacked PR #119 after PR #118; approve only after its exact-SHA CI is green, then retarget/reconcile through canonical `staging` | Requested reviewer: `CubiqoUnited`; Sushma tracks | The author cannot independently approve the operating evidence contract | Live-authority preflight preparation continues; no transaction is allowed | PR #118 is merged, PR #119 is retargeted to `staging`, required checks pass, and latest-head approval is recorded | `merge #119 → run exact-SHA synthetic drills → retain explicitly synthetic receipt → prepare live authority gate` | Controlled-proof preparation and Phase exit; not unrelated Phase work | OPEN |
| HI-P1-SUPPORT-ACCOUNTS | P1 | Staging support, customer-account and native-returns activation | Verify the Resend sender/domain and monitored recipient; provision the three support variables plus the two Shopify Staging public entry URLs in Vercel Preview; enable the native Shopify Staging features | Vercel/Resend/Shopify admin owner; Aarti verifies; Pushpa accepts; Sushma tracks | The authenticated agents can inspect variable names but do not possess the sender/mailbox facts or authority to activate Shopify account/return policy | PR #117 code/tests/runbook and non-live failure drills continue | The five required Preview variables are present, Shopify Staging account/returns are enabled, and the monitored mailbox/operator are identified | Deploy approved PR #117 SHA to canonical Staging, then run no-PII support receipt/failure-alert, account isolation and eligible/ineligible returns drills | Support/account/returns operational proof and Phase exit | OPEN |
| HI-P1-STAGING-ORDER | P1 | Current no-charge Staging payment/order and native cancel/refund proof | Provide protected access through the dedicated Shopify Staging store password gate and confirm the permitted test-payment/admin operator path | Shopify Staging owner; Aarti verifies; Pushpa accepts; Sushma tracks | Public cart creation works, but Shopify redirects checkout to `carlophillips-staging.myshopify.com/password`; agents do not have the password or authenticated Admin session | Tabletop/synthetic exceptions and no-order storefront/cart checks continue | Protected storefront/test-payment access is available without exposing the password in chat/source/logs | Execute one synthetic customer/test-payment order, then native cancellation/refund/restock scenarios without Apliiq Production | Current payment/order and post-sale Staging proof | OPEN |
| HI-P1-NATIVE-OPS | P1 | Shopify/Apliiq webhook subscriptions, Flow alerts and lifecycle reconciliation | Grant or perform authenticated read/config inspection of Shopify/Apliiq/Flow; identify the monitored staff alert recipient and approve the smallest native workflows | Shopify/Apliiq/Flow admin owner; Aarti designs/verifies; Sushma responds | CP has no Shopify Admin token or authenticated provider/Flow session in this task, and native behavior must be established before custom processing | PR #118 proposal, webhook failure tests, local reconciliation fixtures and controlled-order preflight continue | Current subscription/Flow/Apliiq behavior and alert recipient are recorded and approved configuration access exists | Configure/verify native alerts and scheduled exceptions, prove delivery/recovery, then add custom code only for demonstrated residual gaps | Webhook operations, reconciliation, paid-order/Apliiq/tracking watch, Phase exit | OPEN |
