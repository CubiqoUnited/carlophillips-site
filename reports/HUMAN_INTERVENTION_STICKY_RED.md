...
|---|---|---|---|---|---|---|---|---|---|---|---|
| HI-P1-STAGING-VERCEL | ADMIN / EXTERNAL ACCESS REQUIRED | P1 | Protected Staging deployment for `staging@ce2bb181…`, which gates subsequent tranche merges | Resolve Vercel's incompatibility between the approved project-only durable token and unchanged CLI 56.1.0 without broadening token scope or changing workflow architecture; do not expose the value | Vercel platform/admin owner; Aarti determines and verifies the least-privilege compatible credential path; Sushma binds, reruns and closes | The prior secret was an eight-hour CLI session created `2026-09-06T14:54:37Z`, bound `15:12:04Z`, and expired `22:54:37Z`. Org/project/scope IDs and workflow are unchanged. A correctly captured no-expiry `vcp` token scoped only to `prj_9VHD0AhhQnuml8frfNDsmFLHXcq1` was rebound, but exact run `34184480979` attempt 3 failed at `vercel pull` with `User not found (404)`; no build/deploy/alias occurred | PR #117 exact-head checks and non-live preparation continue; #118/#119 stay ordered | The approved durable project-only credential class is supported by the exact protected CLI flow, or Boss explicitly approves the smallest alternate scope/authentication change | Rebind the verified compatible credential, rerun run `34184480979` inputs for PR `116`, SHA `ce2bb181d9a62e5b62af9086c4085b60163077e2`, release `phase1-governance-ce2bb181`, then verify receipt and alias before merging #117 | Canonical Staging verification and all later tranche merges; not local/synthetic preparation | OPEN |
