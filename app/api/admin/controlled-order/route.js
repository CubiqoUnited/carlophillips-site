...
import controlledOrderAuthorization from '@/config/shopify-controlled-order-authorization.json';
...
export async function POST(request) {
  ...
  const releaseEvidence = getProductReleaseEvidence(controlledOrderAuthorization.handle);
  const result = await createControlledMediumCheckout({
    environment: getCommerceEnvironment(),
    controlledOrderAuthorization,
    releaseRecord: releaseEvidence?.releaseRecord,
  });
  if (!result.ok) return noStoreJson({ error: result.reason }, 409);
