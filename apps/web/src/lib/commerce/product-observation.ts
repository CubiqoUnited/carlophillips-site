...
type PartialProductFacts = Partial<ProductFacts>;
type ObservationEnvelope = Omit<
  ProductObservation,
  | 'variantFingerprint'
...
interface FingerprintEnvelope {
  schemaVersion?: ProductObservation['schemaVersion'];
  source?: ProductObservation['source'];
  authority?: ProductObservation['authority'];
  environment?: CommerceEnvironment;
  observedAt?: string;
  capabilityEvidence: string | null;
  product: PartialProductFacts;
  variants: CanonicalVariant[];
}

interface ObservationReviewApproval {
  status: 'approved';
  owner: 'Product Owner' | 'Product Owner/designee';
...
function canonicalDetails(value: unknown): string[] {
...
function canonicalOptions(options: SelectedOption[] = []): SelectedOption[] {
...
function canonicalVariants(
  product: RuntimeProduct & { observedVariants: ObservedVariant[] }
): CanonicalVariant[] {
  return product.observedVariants
    .map((variant) => ({
...
function canonicalSanitizedVariants(
  variants: CanonicalVariant[] = []
): CanonicalVariant[] {
  return variants
    .map((variant) => ({
...
function observationFingerprints(envelope: FingerprintEnvelope) {
  ...
  return {
    ...
    commerceFactsFingerprint: sha256(
      canonicalJson({
...
export function validateProductObservationIntegrity(
  observation: ProductObservation | null | undefined
  ...
  const safeVariants = Array.isArray(observation?.variants)
    ? observation.variants
    : [];
  const canonicalObservedVariants = canonicalSanitizedVariants(safeVariants);
  if (
    canonicalJson(canonicalObservedVariants) !== canonicalJson(safeVariants)
  ) {
  ...
  if (
    expectedFingerprints.variantFingerprint !==
      observation?.variantFingerprint ||
    expectedFingerprints.commerceFactsFingerprint !==
      observation?.commerceFactsFingerprint ||
    expectedFingerprints.observationFingerprint !==
      observation?.observationFingerprint
  ) {
    errors.push('fingerprint integrity mismatch');
  }
  errors.push(
    ...observationFactErrors(observation?.product || {}, safeVariants)
  );
...
function observationFactErrors(
  product: PartialProductFacts,
  variants: CanonicalVariant[]
): string[] {
  const errors: string[] = [];
  ...
  if (
    ...
    canonicalJson(Object.keys(product).sort(compareCanonical)) !==
      canonicalJson(PRODUCT_FACT_KEYS)
  ) {
    errors.push('product fact shape invalid');
  }
  ...
  for (const field of ['description', 'vendor', 'productType', 'tagline']) {
    const value = product[field as keyof ProductFacts];
    if (typeof value !== 'string' || normalizeText(value) !== value) {
      errors.push(`product ${field} invalid`);
    }
  }
  if (
    !Array.isArray(product?.details) ||
    canonicalJson(canonicalDetails(product.details)) !==
      canonicalJson(product.details)
  ) {
    errors.push('product details invalid');
  }
  if (!CURRENCY_PATTERN.test(currency || ''))
    errors.push('product currency invalid');
  if (
    typeof minimumPrice !== 'number' ||
    !Number.isFinite(minimumPrice) ||
    minimumPrice < 0
  )
    errors.push('minimum price invalid');
  if (
    typeof maximumPrice !== 'number' ||
    !Number.isFinite(maximumPrice) ||
    maximumPrice < 0
  )
    errors.push('maximum price invalid');
  if (
    typeof minimumPrice === 'number' &&
    typeof maximumPrice === 'number' &&
    Number.isFinite(minimumPrice) &&
  ...
  if (!Array.isArray(variants) || variants.length === 0)
    return [...errors, 'variants missing'];
  ...
  for (const variant of variants) {
    if (!FINGERPRINT_PATTERN.test(variant?.referenceHash || ''))
      errors.push('variant reference invalid');
    if (references.has(variant?.referenceHash))
      errors.push('duplicate variant reference');
    ...
    if (
      ...
      if (new Set(optionNames).size !== optionNames.length) {
        errors.push('duplicate variant option name');
      }
      const canonicalOptionNames = variant.selectedOptions.map(
        (option) => option.name
      );
      if (expectedOptionNames === null) {
        expectedOptionNames = canonicalOptionNames;
      } else if (
        canonicalJson(canonicalOptionNames) !==
        canonicalJson(expectedOptionNames)
      ) {
        errors.push('variant option dimensions mismatch');
      }
      const combinationKey = canonicalJson(variant.selectedOptions);
      if (optionCombinations.has(combinationKey)) {
        errors.push('duplicate variant option combination');
      }
      optionCombinations.add(combinationKey);
    }
    if (!PRICE_PATTERN.test(variant?.price?.amount || ''))
      errors.push('variant price invalid');
    if (!CURRENCY_PATTERN.test(variant?.price?.currency || ''))
      errors.push('variant currency invalid');
    if (variant?.price?.currency !== currency)
      errors.push('variant currency mismatch');
  ...
  if (variantPrices.length === variants.length) {
    if (Math.min(...variantPrices) !== minimumPrice)
      errors.push('minimum price mismatch');
    if (Math.max(...variantPrices) !== maximumPrice)
      errors.push('maximum price mismatch');
  }
...
export function createProductObservation({
  ...
  if (!['local', 'preview', 'production'].includes(environment)) {
    throw new Error(
      `Unsupported product observation environment: ${environment}`
    );
  }
  ...
  if (source !== 'shopify' && normalizeText(capabilityEvidence)) {
    throw new Error(
      `${source} product observations cannot carry capability evidence.`
    );
  }
  if (source === 'shopify' && !normalizeText(capabilityEvidence)) {
    throw new Error(
      'Shopify product observations require durable product-read capability evidence.'
    );
  }
  if (!product?.handle)
    throw new Error('Product observation requires a handle.');
  if (
    !DATE_TIME_PATTERN.test(observedAt || '') ||
    !Number.isFinite(Date.parse(observedAt))
  ) {
    throw new Error(
      'Product observation requires a valid observedAt timestamp.'
    );
  }
  ...
  const observedProduct = product as RuntimeProduct & {
    observedVariants: ObservedVariant[];
  };
  const variants = canonicalVariants(observedProduct);
  ...
  const productSummary = {
    ...
    details: canonicalDetails(product.details),
  ...
  if (
    ...
    new Set(rawVariants.map((variant) => variant.id)).size !==
      rawVariants.length
  ) {
    throw new Error(
      'Product observation requires unique raw variant references.'
    );
  }
  const factErrors = observationFactErrors(productSummary, variants);
  if (factErrors.length) {
    throw new Error(
      `Product observation facts invalid: ${factErrors.join(', ')}`
    );
  }
...
export function reviewProductObservation({
  ...
  reviewApproval?: ObservationReviewApproval | null;
}) {
  ...
  const approvalReady =
    reviewApproval?.status === 'approved' &&
    ['Product Owner', 'Product Owner/designee'].includes(
      reviewApproval.owner
    ) &&
    reviewApproval.scope === 'accept-shopify-product-observation' &&
    reviewApproval.observationFingerprint ===
      observation?.observationFingerprint &&
  ...
  return {
    ...
    candidateReleasePatch:
      blockers.length === 0 && observation && reviewApproval
        ? {
...
export function attachObservationToProduct(
  product: RuntimeProduct,
