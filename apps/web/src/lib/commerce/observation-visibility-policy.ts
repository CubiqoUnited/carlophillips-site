import { validateProductObservationIntegrity } from './product-observation';
import { createVariantPresentation } from './variant-presentation-policy';
import type {
  CommerceEnvironment,
...
function denied(reason: string): ObservationVisibilityDecision {
  return { ready: false, reason, product: null };
}

export function toReleaseBoundProduct(
...
export function evaluateObservationVisibility({
  ...
  if (environment === 'local') {
    return {
      ready: true,
      reason: null,
      product: toReleaseBoundProduct(shopifyProduct, observation),
    };
  }
  ...
  if (
    observation.variantFingerprint !== releaseRecord.shopify.variantFingerprint
  ) {
