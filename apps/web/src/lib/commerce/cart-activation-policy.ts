...
function prerequisite(
  ...
  resumePoint: string,
  humanRequired = false
): CartActivationDecision['prerequisites'][number] {
  return {
    code,
    status: satisfied
      ? 'satisfied'
...
export function evaluateCartActivation({
  ...
  const fingerprintReady = Boolean(
    ...
    shopifyProduct?.variantFingerprint ===
  ...
  const variantResolverReady =
    variantResolverDecision?.schemaVersion ===
      'cp.variant-resolution-decision.v1' &&
    variantResolverDecision.status === 'ready' &&
    variantResolverDecision.capability ===
      'shopify-storefront-variant-resolver' &&
    variantResolverDecision.adapter ===
      'server-only-shopify-variant-resolver' &&
    ...
    variantResolverDecision.variantFingerprint ===
      releaseRecord?.shopify?.variantFingerprint &&
    variantResolverDecision.variantFingerprint ===
      shopifyProduct?.variantFingerprint &&
  ...
  const fixtureOrLocal =
    environment === 'local' || productDecision?.source === 'fixture';
  const cartAllowed = !fixtureOrLocal && !firstUnmet;
  const checkoutApprovalReady =
    checkoutApproval?.status === 'approved' &&
  ...
  const checkoutAllowed = cartAllowed && checkoutApprovalReady;
  ...
  return {
    ...
    checkoutReason: checkoutAllowed
      ? environment === 'preview'
        ? 'PRIVATE_CHECKOUT_REHEARSAL_AUTHORIZED'
...
export function toCartActivationSummary(
  decision: CartActivationDecision
): CartActivationSummary {
  return {
    ...
    prerequisites: decision.prerequisites.map(
      ({ code, status, resumePoint }) => ({
        code,
        status,
        resumePoint,
      })
