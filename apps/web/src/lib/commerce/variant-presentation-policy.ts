...
export function createVariantPresentation(
  ...
  const variants = Array.isArray(observation?.variants)
...
export function hasSellableReviewedVariant(
  presentation: VariantPresentation | null | undefined,
  {
    expectedVariantFingerprint,
    productVariantFingerprint,
    productCurrency,
  }: {
    expectedVariantFingerprint?: string;
  ...
  for (const combination of presentation.combinations) {
    ...
    if (
      ...
      optionNames.some(
        (name, index) => name !== presentation.optionNames[index]
      ) ||
