...
function readyProductReadCapability(
  decision: CapabilityDecision | null,
  observation: ProductObservation | undefined
): boolean {
  return (
...
function readyLocalResolverCapability(
  decision: CapabilityDecision | null
): boolean {
  return (
    decision?.status === 'ready' &&
...
export function evaluateVariantResolutionReadiness({
  ...
  productReadCapabilityDecision: CapabilityDecision;
  resolverCapabilityDecision: CapabilityDecision;
}): VariantResolutionDecision {
  ...
  if (
    !hasSellableReviewedVariant(product?.variantPresentation, {
  ...
  let regeneratedObservation: ProductObservation | null = null;
  try {
    regeneratedObservation = createProductObservation({
      source: 'shopify',
      environment: observation?.environment as CommerceEnvironment,
  ...
  if (
    ...
    regeneratedObservation.variantFingerprint !==
  ...
  const reviewedReferences = new Set<string>(
    product?.variantPresentation?.combinations?.map(
      (item) => item.referenceHash
    ) || []
  );
  ...
  const oneToOneMapping =
    reviewedReferences.size > 0 &&
    reviewedReferences.size === currentReferences.size &&
    [...reviewedReferences].every((reference) =>
      currentReferences.has(reference)
    );
  if (!oneToOneMapping) {
    blockers.push(
      blocker(
  ...
  return {
    ...
    variantFingerprint: FINGERPRINT_PATTERN.test(expectedFingerprint || '')
      ...
      : null,
    evidenceRef: ready
      ? (resolverCapabilityDecision.evidenceRef ?? null)
      : null,
    productReadEvidenceRef: ready
      ? (productReadCapabilityDecision.evidenceRef ?? null)
      : null,
