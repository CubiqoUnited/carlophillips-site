...
const ALLOWED_TRANSITIONS: Record<ReleaseState, Set<ReleaseState>> = {
  draft: new Set(['staged']),
  staged: new Set(['draft', 'approved']),
  approved: new Set(['staged', 'released']),
  released: new Set(['withdrawn']),
  withdrawn: new Set<ReleaseState>(),
};

const FINGERPRINT_PATTERN = /^sha256:[a-f0-9]{64}$/;
const COMMIT_PATTERN = /^[a-f0-9]{7,40}$/;

function blocker(code: string, humanAction: string, resumePoint: string): Blocker {
...
function addStagingBlockers(record: ReleaseRecord, blockers: Blocker[]): void {
  if (
...
function addApprovalBlockers(
  ...
  if (
    ...
    || !hasText(sample.sampleReference)
    || !hasText(sample.evidence)
    || (['fit', 'colour', 'artworkPlacement', 'finish'] as const).some(check => sample?.[check] !== 'approved')
  ) {
    blockers.push(blocker(
      'PHYSICAL_SAMPLE_APPROVAL_REQUIRED',
      'Order and inspect the exact physical sample, then approve fit, colour, artwork placement, and finish.',
      'Bind the exact sample reference and inspection evidence to the Product Release Record, then re-evaluate approval.',
    ));
  }
  for (const [field, code, label] of [
    ['responsiveEvidence', 'RESPONSIVE_QA_EVIDENCE_REQUIRED', 'desktop and mobile Preview QA'],
    ['performanceEvidence', 'PERFORMANCE_EVIDENCE_REQUIRED', 'media loading and performance QA'],
    ['tokenEvidence', 'DESIGN_TOKEN_EVIDENCE_REQUIRED', 'design-token and visual-regression QA'],
  ] as const) {
    if (!hasText(record.candidate?.[field])) {
      blockers.push(blocker(
        code,
        `Complete ${label} for this exact candidate.`,
        `Bind the passing ${label} evidence to the Product Release Record, then re-evaluate approval.`,
      ));
    }
  }
...
function addReleaseBlockers(record: ReleaseRecord, blockers: Blocker[]): void {
...
export function evaluateProductReleaseTransition({
  record,
  manifest,
  targetState,
}: {
...
export function evaluateProductReleaseEvidence({
  record,
  manifest,
