...
const brandAssets = {
  ...
  collections: {
    ...
    banners: {
      ...
      grooming: {
        url: null,
        fallback:
          'https://images.unsplash.com/photo-1581750216968-5a5dca94588e?w=1200&q=80',
  ...
  about: {
    hero: {
      url: null,
      fallback:
        'https://images.unsplash.com/photo-1698306871917-7b91b07a0bb4?w=1600&q=80',
  ...
  social: {
    ...
    twitterImage: {
      url: null,
      fallback:
        'https://images.unsplash.com/photo-1441984904996-e0b6ba687e04?w=1200&h=600&fit=crop&q=80',
      width: 1200,
      height: 600,
      alt: 'CARLOPHILLIPS - Quiet Luxury',
    },
  ...
  placeholders: {
    ...
    product:
      'https://images.unsplash.com/photo-1441984904996-e0b6ba687e04?w=600&h=800&fit=crop&q=80',
...
export function getAssetUrl(
  asset: { url?: string | null; fallback?: string } | null | undefined
): string {
  if (!asset) return '';
 ...
 */
export function getCollectionBanner(
  collectionId: keyof typeof brandAssets.collections.banners
) {
...
export function getPlaceholder(
  type: keyof typeof brandAssets.placeholders = 'product'
): string {
...
export function getBrandAssetStatus() {
  return {
    ...
    hasCollectionBanners: Object.values(brandAssets.collections.banners).some(
      (b) => b.url
    ),
    hasOgImage: Boolean(brandAssets.social.ogImage.url),
    totalCampaignImages: brandAssets.campaign.stills.length,
    configuredCampaignImages: brandAssets.campaign.stills.filter(
      (img) => img.url
    ).length,
  };
}
