...
describe('Funnel 2 provider handshakes', () => {
  it('covers every requested provider without claiming an authenticated connection', () => {
    ...
    expect(
      rows.every(
        (row) => row.sideEffects === 'none' && row.costIncurred === false
      )
    ).toBe(true);
  ...
  it('treats a configured API secret as credential presence, not proof of authentication', () => {
    const rows = deriveProviderHandshakes({
      registry,
      environment: { RUNWAYML_API_SECRET: 'server-only-placeholder' },
    });
    expect(rows.find((row) => row.id === 'runway')).toMatchObject({
  ...
  it('fails closed for duplicate, malformed or unknown provider records', () => {
    ...
    expect(() => deriveProviderHandshakes({ registry: duplicate })).toThrow(
      'MEDIA_PROVIDER_HANDSHAKE_REGISTRY_INVALID'
    );
    expect(() =>
      deriveProviderHandshakes({
        registry: { schemaVersion: 'wrong', providers: [] },
      })
