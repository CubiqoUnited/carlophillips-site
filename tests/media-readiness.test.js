...
import {
  createFileProbe,
  getMediaReadiness,
} from '../lib/media/media-readiness-server.js';

const slotById = (id) =>
  MEDIA_READINESS_SLOTS.find((slot) => slot.slotId === id);
const decisionById = (readiness, id) =>
  readiness.decisions.find((decision) => decision.slotId === id);
...
describe('media readiness contract', () => {
  it('declares the landing hero and the three approved product clips', () => {
    expect(
      MEDIA_READINESS_SLOTS.filter((slot) => slot.role === 'landing-hero').map(
        (slot) => slot.aspect.label
      )
    ).toEqual(['16:9', '9:16']);
    expect(
      MEDIA_READINESS_SLOTS.filter((slot) => slot.role === 'product-video').map(
        (slot) => slot.label
      )
    ).toEqual(['Fit & silhouette', 'Runway motion', '360 showcase']);
    for (const slot of MEDIA_READINESS_SLOTS.filter(
      (slot) => slot.role === 'product-video'
    )) {
  ...
  it('marks a slot ready only when an approved source and a first-frame poster are both evidenced', () => {
    const readiness = evaluateMediaReadiness({
      probe: probeFrom(healthyEntries),
      slots: [runwaySlot],
    });
  ...
  it('refuses a source that is present but not real ISO media, and one that is truncated', () => {
    ...
    expect(decisionById(notMedia, 'product-runway-motion').reason).toBe(
      MEDIA_READINESS_REASON.sourceNotIsoMedia
    );

    const truncated = evaluateMediaReadiness({
      probe: probeFrom({
        ...healthyEntries,
        [runwaySlot.source.path]: { exists: true, bytes: 12, isoMedia: true },
      }),
      slots: [runwaySlot],
    });
    expect(decisionById(truncated, 'product-runway-motion').reason).toBe(
      MEDIA_READINESS_REASON.sourceEmpty
    );
  });
  ...
  it('refuses a renderable source that has no reduced-motion poster', () => {
    const readiness = evaluateMediaReadiness({
      probe: probeFrom({
        [runwaySlot.source.path]: healthyEntries[runwaySlot.source.path],
      }),
      slots: [runwaySlot],
    });
    const decision = decisionById(readiness, 'product-runway-motion');
  ...
  it('rejects a master whose observed aspect contradicts the declared composition', () => {
    ...
    expect(decisionById(readiness, 'product-runway-motion').reason).toBe(
      MEDIA_READINESS_REASON.aspectMismatch
    );
  });
  ...
  it('fails closed for a declared slot with no provisioned master or poster', () => {
    const readiness = evaluateMediaReadiness({
      probe: probeFrom({}),
      slots: [slotById('product-360-showcase')],
    });
    ...
    expect(readiness.blocking).toEqual([
      {
        ...
        role: 'product-video',
        reason: MEDIA_READINESS_REASON.sourceNotDeclared,
      },
    ]);
  });
  ...
  it('prefers an adaptive playback identifier over a local master when one is provisioned', () => {
    const slot = {
      ...runwaySlot,
      source: { ...runwaySlot.source, playbackId: 'abc123' },
    };
    const readiness = evaluateMediaReadiness({
      probe: probeFrom({
        [runwaySlot.posters[0].path]: { exists: true, bytes: 15_000 },
  ...
  it('reports the repository state: two approved clips ready, the 360 slot withheld, hero poster-only', () => {
    ...
    expect(decisionById(readiness, 'product-runway-motion').verdict).toBe(
      MEDIA_READINESS_VERDICT.ready
    );
    expect(decisionById(readiness, 'product-fit-silhouette').verdict).toBe(
      MEDIA_READINESS_VERDICT.ready
    );
    expect(decisionById(readiness, 'product-360-showcase').verdict).toBe(
      MEDIA_READINESS_VERDICT.notReady
    );
    expect(readiness.landingHero.renderable).toBe(true);
    expect(heroPresentation(readiness, 'desktop').posterUrl).toMatch(
      /lofoten-runway-hero/
    );
    expect(readyProductClips(readiness).map((clip) => clip.label)).toEqual([
      'Fit & silhouette',
      'Runway motion',
    ]);
  });
  ...
  it('probes real files, sizes them, and detects the ISO media brand', () => {
    ...
    expect(
      probe('public/media/signature-hoodie/videos/does-not-exist.mp4')
