...
function append(aggregate, eventType, overrides = {}) {
  return recordOrderLifecycleEvent(aggregate, nextEvent(aggregate, eventType, overrides));
}

function acceptedOrder() {
  let aggregate = create();
  aggregate = append(aggregate, 'payment-authorized', { source: 'commerce-gateway' });
  return append(aggregate, 'order-accepted', { source: 'commerce-gateway' });
}

function deliveredOrder() {
  let aggregate = acceptedOrder();
  aggregate = append(aggregate, 'pod-handoff-accepted', { source: 'fulfillment-gateway' });
  aggregate = append(aggregate, 'production-started', { source: 'fulfillment-gateway' });
...
describe('provider-neutral order lifecycle', () => {
  it('reduces authorized payment through delayed delivery, review eligibility, and reconciliation', () => {
    ...
    aggregate = append(aggregate, 'review-eligible', { source: 'support-gateway' });
    aggregate = append(aggregate, 'reconciled', { source: 'reconciliation-engine' });
    ...
    expect(aggregate.events.every((event, index) => (
      event.sequence === index + 1
      && event.previousEventHash === (index ? aggregate.events[index - 1].eventHash : null)
    ))).toBe(true);
  });

  it('suppresses an exact replay but rejects an idempotency conflict', () => {
    const initial = create();
    const event = nextEvent(initial, 'payment-authorized', { source: 'commerce-gateway' });
    const once = recordOrderLifecycleEvent(initial, event);
    expect(recordOrderLifecycleEvent(once, event)).toEqual(once);

    const conflict = buildOrderLifecycleEvent({ ...event, eventId: 'evt-lifecycle-conflict' });
    expectCode(() => recordOrderLifecycleEvent(once, conflict), 'IDEMPOTENCY_CONFLICT');
    expectCode(() => recordOrderLifecycleEvent(once, { ...event, customerEmail: 'person@example.test' }), 'INVALID_EVENT_ENVELOPE');
  });

  it('rejects tampering, stale sequence, broken chain, reused IDs, and out-of-order time', () => {
    const initial = create();
    const firstEvent = nextEvent(initial, 'payment-authorized', { source: 'commerce-gateway' });
    const once = recordOrderLifecycleEvent(initial, firstEvent);

    expectCode(() => recordOrderLifecycleEvent(initial, { ...firstEvent, eventHash: hash('0') }), 'EVENT_HASH_REJECTED');
    expectCode(() => recordOrderLifecycleEvent(once, nextEvent(once, 'order-accepted', { sequence: 1 })), 'EVENT_SEQUENCE_REJECTED');
    expectCode(() => recordOrderLifecycleEvent(once, nextEvent(once, 'order-accepted', { previousEventHash: null })), 'EVENT_CHAIN_REJECTED');
    expectCode(() => recordOrderLifecycleEvent(once, nextEvent(once, 'order-accepted', {
  ...
  it('rejects cross-release, cross-variant, and cross-environment events', () => {
    ...
    for (const eventBinding of [
      ...
      expectCode(() => recordOrderLifecycleEvent(aggregate, nextEvent(aggregate, 'payment-authorized', {
  ...
  it('requires exact Released, Production, checkout, and controlled-order authority', () => {
    ...
    expectCode(() => recordOrderLifecycleEvent(preview, nextEvent(preview, 'payment-authorized', { source: 'commerce-gateway' })), 'PRODUCTION_SALE_EVENT_REQUIRED');

    const aggregate = create();
    for (const [authorityOverride, code] of [
      [{ releaseState: 'draft' }, 'RELEASED_AUTHORITY_REQUIRED'],
      [{ checkoutAuthorizationFingerprint: null }, 'CHECKOUT_AUTHORIZATION_REQUIRED'],
  ...
  it.each(['release-approved', 'checkout-authorized', 'refund-approved', 'publication-released'])(
    'cannot use lifecycle event %s to create upstream authority',
    eventType => {
      const aggregate = create();
      expectCode(() => recordOrderLifecycleEvent(aggregate, nextEvent(aggregate, eventType)), 'INVALID_EVENT_ENVELOPE');
    }
  );
  ...
  it('rejects PII, raw references, arbitrary fields, and malformed money', () => {
    ...
    for (const details of [
      ...
      expect(() => recordOrderLifecycleEvent(aggregate, nextEvent(aggregate, 'payment-authorized', {
        source: 'commerce-gateway',
        details,
      }))).toThrow();
    }
  ...
  it('records payment failure but blocks order acceptance', () => {
    ...
    expectCode(() => recordOrderLifecycleEvent(aggregate, nextEvent(aggregate, 'order-accepted', { source: 'commerce-gateway' })), 'ORDER_REQUIRES_AUTHORIZED_PAYMENT');
  });
  ...
  it('records POD rejection but blocks production', () => {
    ...
    expectCode(() => recordOrderLifecycleEvent(aggregate, nextEvent(aggregate, 'production-started', { source: 'fulfillment-gateway' })), 'PRODUCTION_REQUIRES_POD_ACCEPTANCE');
  });
  ...
  it('gates review eligibility while support is open', () => {
    ...
    aggregate = append(aggregate, 'support-opened', {
      ...
      details: { supportCaseReferenceHash: hash('2'), reasonCode: 'DELIVERY_QUESTION' },
    });
    expectCode(() => recordOrderLifecycleEvent(aggregate, nextEvent(aggregate, 'review-eligible', { source: 'support-gateway' })), 'REVIEW_ELIGIBILITY_GATES_FAILED');
    aggregate = append(aggregate, 'support-resolved', { source: 'support-gateway' });
    aggregate = append(aggregate, 'review-eligible', { source: 'support-gateway' });
    expect(aggregate.state).toMatchObject({ support: 'resolved', review: 'eligible' });
  });

  it('requires a received return and exact refund authority for partial/full refunds', () => {
    let aggregate = deliveredOrder();
    aggregate = append(aggregate, 'return-requested', { source: 'support-gateway' });
    aggregate = append(aggregate, 'return-approved', { source: 'operator' });
    aggregate = append(aggregate, 'return-received', { source: 'fulfillment-gateway' });

    expectCode(() => recordOrderLifecycleEvent(aggregate, nextEvent(aggregate, 'refund-issued-partial', {
      source: 'commerce-gateway',
    ...
    expect(aggregate.state).toMatchObject({ payment: 'refunded_full', return: 'received', review: 'ineligible' });
    expectCode(() => recordOrderLifecycleEvent(aggregate, nextEvent(aggregate, 'review-eligible', { source: 'support-gateway' })), 'REVIEW_ELIGIBILITY_GATES_FAILED');
  });
  ...
  it('records bounded reconciliation variance without granting commerce authority', () => {
    ...
    aggregate = append(aggregate, 'reconciliation-variance', {
      ...
      details: { varianceType: 'amount', amount: { amount: '1.00', currency: 'USD' } },
    });
    expect(aggregate.state.reconciliation).toBe('variance');
    expect(deriveOrderLifecycleAdminProjection(aggregate).authoritative).toBe(false);
  });
  ...
  it('derives truthful empty and populated admin projections without hashes or idempotency keys', () => {
    ...
    expect(JSON.stringify(projection)).not.toMatch(/sha256:|idempotency|aggregateReference|releaseFingerprint|variantFingerprint/);
  });
});
