...
} from '../lib/analytics/browser-loader.js';
...
describe('production customer experience authority', () => {
  ...
  it('uses deterministic sitemap evidence and excludes unreleased products', () => {
    ...
    expect(
      entries.every(
        (entry) =>
          entry.lastModified.toISOString() === '2026-08-14T00:00:00.000Z'
      )
    ).toBe(true);
    expect(entries.some((entry) => entry.url.includes('/products/'))).toBe(
      false
    );
  });
  ...
  it('keeps draft policy content out of the sitemap', () => {
    expect(
      sitemap().some((entry) =>
        /\/(privacy|terms|cookie-policy)$/.test(entry.url)
      )
    ).toBe(false);
  });

  it('publishes all technical policy surfaces', () => {
    for (const route of ['privacy', 'terms', 'cookie-policy'])
      expect(existsSync(`app/${route}/page.js`)).toBe(true);
  });

  it('connects privacy-preserving public observability without restoring a consent banner', () => {
    expect(existsSync('components/privacy/consent-preferences.jsx')).toBe(
      false
    );
    const layout = readFileSync('app/layout.js', 'utf8');
    const observability = readFileSync(
      'components/analytics/site-observability.jsx',
      'utf8'
    );
  ...
  it('validates GA IDs and stays silent before every independent gate passes', () => {
    ...
    for (const value of ['', 'UA-123-4', 'G-ABC;alert(1)', 'G-short'])
  ...
  it('rejects unsafe analytics properties and unsupported events', () => {
    expect(
      createAnalyticsEvent('product_view', {
        route_group: 'product',
        raw_product_id: 'gid://secret',
        email: 'person@example.com',
      })
    ).toEqual({ name: 'product_view', properties: { route_group: 'product' } });
    expect(() => createAnalyticsEvent('checkout_identity')).toThrow(
