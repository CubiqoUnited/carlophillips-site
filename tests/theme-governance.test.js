...
describe('canonical theme token governance', () => {
  it('keeps exactly four validated values in one canonical repository file', () => {
    ...
    expect(parseThemeDocument('{broken').errors).toContain('Theme JSON is malformed.');
  });

  it('enforces deterministic accent contrast on both canonical dark canvases', () => {
    expect(contrastRatio(theme.accentColor, '#000000')).toBeGreaterThanOrEqual(4.5);
    expect(contrastRatio(theme.accentColor, '#020202')).toBeGreaterThanOrEqual(4.5);
  ...
  it('rejects stale writes, accepts unchanged writes, and prepares only validated changes', () => {
    ...
    expect(prepareThemeWrite({ candidate: theme, currentTheme: theme, expectedFingerprint: fingerprint }))
      .toMatchObject({ ok: true, code: 'THEME_UNCHANGED', fingerprint });
    expect(prepareThemeWrite({ candidate: theme, currentTheme: theme, expectedFingerprint: `sha256:${'0'.repeat(64)}` }))
      .toMatchObject({ ok: false, status: 409, code: 'THEME_REVISION_STALE' });
    expect(prepareThemeWrite({ candidate: { ...theme, textWeight: 400 }, currentTheme: theme, expectedFingerprint: fingerprint }))
      .toMatchObject({ ok: true, code: 'THEME_BRANCH_PROPOSAL_SAVED' });
    expect(prepareThemeWrite({ candidate: { ...theme, baseSpacing: '9rem' }, currentTheme: theme, expectedFingerprint: fingerprint }))
      .toMatchObject({ ok: false, status: 422, code: 'THEME_VALIDATION_FAILED' });
  });
  ...
  it('allows writes only on a local codex branch and rejects CSRF/external surfaces', () => {
    ...
    expect(evaluateThemeWriteBoundary({ ...base, branch: 'main' }).reason).toBe('temporary_feature_branch_required');
    expect(evaluateThemeWriteBoundary({ ...base, writeEnabled: false }).reason).toBe('theme_writes_disabled');
    expect(evaluateThemeWriteBoundary({ ...base, runtimeSurface: 'vercel-preview' }).reason).toBe('external_surface_denied');
    expect(evaluateThemeWriteBoundary({ ...base, runtimeSurface: 'vercel-production' }).reason).toBe('external_surface_denied');
    expect(evaluateThemeWriteBoundary({ ...base, commerceEnvironment: 'production' }).reason).toBe('commerce_environment_denied');
  ...
  it('rejects a symlinked canonical file and cannot resolve a caller-selected target path', () => {
    ...
    symlinkSync(path.join(outsideRoot, 'theme.json'), path.join(repositoryRoot, 'theme.json'));
    try {
      expect(() => resolveCanonicalThemeTarget(repositoryRoot)).toThrow('THEME_TARGET_MUST_NOT_BE_A_SYMLINK');
    } finally {
  ...
  it('atomically replaces only the canonical file with a validated serialization', () => {
    ...
    try {
      ...
      expect(readFileSync(locations.target, 'utf8')).toBe(serializeTheme(proposedTheme));
  ...
  it('extends hardcoded-value enforcement across every active app and component source', () => {
    ...
    for (const file of files) {
      ...
      if (/#[0-9a-f]{3,8}\b|rgba?\(/i.test(source)) violations.push(`${file}: raw colour`);
      for (const match of source.matchAll(/(?:border-radius|font-weight|(?:margin|padding|gap)(?:-[a-z]+)?):\s*([^;]+);/g)) {
        if (!match[1].includes('var(--cp-')) violations.push(`${file}: ${match[0]}`);
      }
    }
    expect(violations).toEqual([]);

    const editor = readFileSync('components/admin/theme-editor.jsx', 'utf8');
    expect(editor).toContain("'--cp-theme-preview-accent': proposed.accentColor");
    expect(editor).not.toMatch(/(?:accentColor|cornerRadius|baseSpacing|textWeight):\s*(?:['"](?:#|\d)|\d)/);
    expect(readFileSync('components/admin/control-plane.jsx', 'utf8')).toContain('<Status value="branch_proposal" />');
    expect(readFileSync('app/admin/admin.module.css', 'utf8')).not.toMatch(/\.status[^}]*color-accent/s);
  });
  ...
  it('keeps Tailwind removed and binds any future adapter to the same JSON authority', () => {
    ...
    expect(readFileSync('docs/design-system.md', 'utf8')).toContain('future Tailwind adapter');
    expect(readFileSync('docs/design-system.md', 'utf8')).toContain('theme.json');
  });
});
