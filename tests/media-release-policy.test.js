...
describe('media release policy', () => {
  ...
  it('accepts an approved infeasibility only for a where-feasible modality', () => {
    ...
    manifest.assets = manifest.assets.filter(asset => asset.assetId !== 'model-asset');
  ...
  ])('rejects an approved requirement whose asset has unsafe %s', (field, value, code) => {
    const manifest = createCompleteMediaManifest();
    const asset = manifest.assets.find(item => item.assetId === 'front-image');
    asset[field] = value;
    const decision = evaluateMediaRelease(manifest);
    expect(decision.ready).toBe(false);
    expect(decision.assetBlockers).toContainEqual({ assetId: 'front-image', code });
  });

  it('requires verified asset quality evidence', () => {
    const manifest = createCompleteMediaManifest();
    const asset = manifest.assets.find(item => item.assetId === 'material-detail-image');
  ...
  it('rejects a missing or duplicate modality in the required matrix', () => {
    ...
    missing.requirements = missing.requirements.filter(item => item.modality !== 'lifestyle');
  ...
  it('does not allow required video to be downgraded to an infeasibility exception', () => {
    ...
    Object.assign(video, {
      ...
      assetIds: [],
      infeasibilityBlocker: {
        reason: 'A required product film cannot be waived through a where-feasible policy.',
        approvalStatus: 'approved',
        owner: 'Product Owner',
      },
    });
