...
describe('canonical evidence reconciliation', () => {
  ...
  it('is deterministic and classifies mutable evidence against an explicit date', () => {
    ...
    expect(classifyFreshness('2026-08-04', '2026-08-14')).toEqual({
      status: 'stale',
      ageDays: 10,
    });
    expect(classifyFreshness(null, '2026-08-14')).toEqual({
      status: 'not_recorded',
      ageDays: null,
    });
  });
  ...
  it('supersedes the old login blocker without claiming the later evidence is current authority', () => {
    expect(
      model.records.find((record) => record.id === 'shopify-session-access')
    ).toMatchObject({
    ...
    expect(
      model.runRows.find((item) => item.id === 'shopify-live-readonly-audit')
    ).toMatchObject({
  ...
  it('keeps the no-order cart test historical, technical-only, and blocked', () => {
    expect(
      model.records.find((record) => record.id === 'shopify-cart-test')
    ).toMatchObject({
    ...
    expect(
      model.capabilityRows.find((item) => item.id === 'shopify-storefront-cart')
    ).toMatchObject({
  ...
  it('keeps a Staged release review-bound but never operationally ready', () => {
    ...
    expect(
      model.records.find(
        (record) => record.id === 'release-observation-binding'
      )
    ).toMatchObject({
  ...
  it('reports static run history instead of inventing an append-only audit chain', () => {
    expect(
      model.records.find((record) => record.id === 'operational-audit-chain')
    ).toMatchObject({
