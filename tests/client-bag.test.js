...
} from '../lib/site/contact-intake.js';
...
describe('client bag', () => {
  it('adds, merges by variant, changes quantity and removes', () => {
    ...
    const two = addLine(merged, {
      ...line,
      referenceHash: 'sha256:bbb',
      size: 'L',
    });
    ...
    expect(
      setQuantity(raised, bagLineKey(raised.lines[1]), 0).lines
    ).toHaveLength(1);
    expect(
      removeLine(two, 'sha256:aaa').lines.map((item) => item.size)
    ).toEqual(['L']);
  ...
  it('rejects a stored bag that does not match the current schema', () => {
    ...
    expect(
      readBag({ schemaVersion: BAG_SCHEMA, lines: [line, { handle: 'x' }] })
        .lines
    ).toHaveLength(1);
    expect(
      readBag({ schemaVersion: BAG_SCHEMA, lines: [{ ...line, quantity: -3 }] })
        .lines[0].quantity
    ).toBe(1);
  });
  ...
  it('leaves the total unchanged for a code it does not recognise', () => {
    ...
    expect(rejected.discount).toMatchObject({
      code: 'ANYTHING',
      status: DISCOUNT_STATUS.rejected,
    });
    expect(bagTotals(rejected).total).toBe(180);

    const applied = applyDiscount(bag, 'cp10', [
      { code: 'CP10', percentage: 10 },
    ]);
    expect(applied.discount.status).toBe(DISCOUNT_STATUS.applied);
    expect(bagTotals(applied)).toMatchObject({
      subtotal: 180,
      discountAmount: 18,
      total: 162,
    });
    expect(applyDiscount(bag, '   ', []).discount.status).toBe(
...
describe('discovery catalog overlays', () => {
  ...
  it('derives counts from what the release actually made visible', () => {
    ...
    expect(cards.find((card) => card.id === 'hoodies')).toMatchObject({
      meta: '1 item',
      available: true,
      viewing: true,
    });
  ...
  it('shows nothing when the release decision withheld the product', () => {
    expect(
      releasedProductCards({ visibleCount: 0, primaryProduct: null })
    ).toEqual([]);
    expect(
      discoveryProductCards({ visibleCount: 0, primaryProduct: null })
    ).toEqual([]);
    for (const card of discoveryCategoryCards({
      visibleCount: 0,
      primaryProduct: null,
    })) {
  ...
  it('prices the product grid from the release decision', () => {
    const [card] = discoveryProductCards(
      summary,
      'hoodies',
...
describe('support and private-list intake', () => {
  it('requires name, a real email and a message', () => {
    expect(
      validateSupportRequest({ name: 'A', email: 'a@b.co', message: 'Hi' })
    ).toEqual({ valid: true, missing: [] });
    expect(
      validateSupportRequest({ name: ' ', email: 'nope', message: '' }).missing
  ...
  it('never reports a request as sent without a configured destination', async () => {
    const { supportDestination, privateListDestination } =
      await import('../lib/site/contact-intake.js');
    expect(supportDestination({})).toBe(null);
    expect(
      supportDestination({ CP_SUPPORT_WEBHOOK_URL: 'http://insecure.example' })
    ).toBe(null);
    expect(
      supportDestination({
        CP_SUPPORT_WEBHOOK_URL: 'https://intake.example/support',
      })
    ).toBe('https://intake.example/support');
