        ...
        data: {
          cartCreate: {
            cart: {
              ...
              totalQuantity: 1,
              cost: { subtotalAmount: { amount: '128.00', currencyCode: 'USD' } },
            },
            userErrors: [],
          },
        },
...
describe('Product Owner controlled Medium checkout', () => {
  ...
  it.each([
    ['missing authorization', { controlledOrderAuthorization: null }, 'CONTROLLED_ORDER_AUTHORIZATION_REQUIRED'],
    ['expired authorization', { now: () => new Date('2026-08-25T00:00:00Z') }, 'CONTROLLED_ORDER_AUTHORIZATION_REQUIRED'],
    ['disabled gate', { controlledOrderRequested: false }, 'CONTROLLED_ORDER_ENVIRONMENT_GATE_DISABLED'],
    ['local surface', { environment: 'local' }, 'CONTROLLED_ORDER_AUTHORIZATION_REQUIRED'],
    ['unverified cart evidence', { capabilityRegistry: { capabilities: [] } }, 'CONTROLLED_ORDER_CART_CAPABILITY_NOT_VERIFIED'],
  ])('fails closed for %s', async (_label, override, reason) => {
    const input = options(override);
    await expect(createControlledMediumCheckout(input)).resolves.toEqual({ ok: false, reason });
    expect(input.fetchImpl).not.toHaveBeenCalled();
  });

  it('rejects an unreviewed provider mapping before Shopify access', async () => {
    const input = options();
    input.releaseRecord.fulfillmentMappings[0].mappingFingerprintStatus = 'observed';
  ...
  it('rejects stale Shopify facts and a changed Medium price before cart creation', async () => {
    const stale = options();
    stale.loadProductImpl = vi.fn(async () => currentProduct(stale.releaseRecord, {
      observation: {
        variantFingerprint: `sha256:${'0'.repeat(64)}`,
        commerceFactsFingerprint: stale.releaseRecord.shopify.commerceFactsFingerprint,
      },
    }));
    ...
    changedSubtotal.fetchImpl = vi.fn(async () => ({
      ...
      json: async () => ({
        data: { cartCreate: { cart: {
          checkoutUrl: 'https://example.myshopify.com/checkouts/controlled',
          totalQuantity: 1,
          cost: { subtotalAmount: { amount: '129.00', currencyCode: 'USD' } },
        }, userErrors: [] } },
      }),
    }));
    await expect(createControlledMediumCheckout(changedSubtotal)).resolves.toEqual({
      ok: false,
