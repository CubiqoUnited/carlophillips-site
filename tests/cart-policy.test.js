...
describe('commerce cart policy', () => {
  it('applies deterministic add, update, and remove transitions without mutating input', () => {
    ...
    expect(added).toMatchObject({ total: 125, subtotal: 125, totalQuantity: 1 });
  ...
  it('rejects invalid local quantities', () => {
    expect(() => addLocalCartLine(createEmptyCart(), { ...line, quantity: 0 })).toThrow(TypeError);
    expect(() => updateLocalCartLine(createEmptyCart(), line.key, 1.5)).toThrow(TypeError);
  });

  it.each(['preview', 'production'])('forbids local carts in %s', environment => {
    expect(() => assertLocalCartAllowed(environment)).toThrow(expect.objectContaining({
  ...
  it('restores a valid Shopify cart', async () => {
    ...
    const result = await restoreOrCreateCart({ cartId: 'opaque-cart', getCart, createCart });
  ...
  ])('creates a Shopify cart when the stored cart is %s', async (_label, restoredValue) => {
    const getCart = restoredValue instanceof Error
      ? vi.fn().mockRejectedValue(restoredValue)
      : vi.fn().mockResolvedValue(restoredValue);
    const createCart = vi.fn().mockResolvedValue({ id: 'new-opaque-cart', items: [] });
    const result = await restoreOrCreateCart({ cartId: 'old-opaque-cart', getCart, createCart });
    expect(result).toMatchObject({ disposition: 'replaced', cart: { id: 'new-opaque-cart' } });
  });

  it('allows only exact HTTPS checkout hosts', () => {
    const hosts = parseCheckoutHosts('checkout.shopify.com, shop.example.com');
    expect(validateCheckoutUrl('https://checkout.shopify.com/c/abc', hosts))
      .toBe('https://checkout.shopify.com/c/abc');
    expect(validateCheckoutUrl('http://checkout.shopify.com/c/abc', hosts)).toBe('');
    expect(validateCheckoutUrl('https://checkout.shopify.com.evil.test/c/abc', hosts)).toBe('');
