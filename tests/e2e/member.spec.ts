...
for (const viewport of [
  ...
  test(`Aftercare is truthful and usable at ${viewport.name} width`, async ({
    ...
    await expect(
      page.getByRole('heading', { name: 'Access your order securely.' })
    ).toBeVisible();
    await expect(page.getByText('Shopify authoritative')).toHaveCount(0);
    await expect(page.getByText('Fit memory')).toHaveCount(0);
