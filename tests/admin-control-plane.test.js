...
describe('local read-only admin access', () => {
  it('denies absent, disabled, short-token, malformed, and external requests', () => {
    const base = {
      ...
      expectedProductOwnerToken: validOwnerToken,
      reviewEnabled: true,
      runtimeSurface: 'local',
    };
    expect(evaluateAdminAccess({ ...base, authorization: null }).allowed).toBe(
      false
    );
    expect(evaluateAdminAccess({ ...base, reviewEnabled: false }).allowed).toBe(
      false
    );
    expect(
      evaluateAdminAccess({ ...base, expectedToken: 'short' }).allowed
    ).toBe(false);
    expect(
      evaluateAdminAccess({ ...base, authorization: `Basic ${validToken}` })
        .allowed
    ).toBe(false);
    expect(
      evaluateAdminAccess({ ...base, runtimeSurface: 'vercel-preview' }).allowed
    ).toBe(false);
    expect(
      evaluateAdminAccess({ ...base, runtimeSurface: 'vercel-production' })
        .allowed
    ).toBe(false);
  });
  ...
  it('allows only the exact configured local bearer token', () => {
    ...
    expect(
      evaluateAdminAccess({ ...base, authorization: `Bearer ${validToken}` })
    ).toEqual({
      allowed: true,
    ...
    expect(
      evaluateAdminAccess({ ...base, authorization: `Bearer ${validToken}x` })
        .allowed
    ).toBe(false);
  });
  ...
  it('requires a distinct Product Owner credential and denies every non-local surface', () => {
    ...
    expect(evaluateAdminAccess(base)).toEqual({
      allowed: true,
    ...
    expect(
      evaluateAdminAccess({ ...base, authorization: `Bearer ${validToken}` })
    ).toMatchObject({ allowed: false, reason: 'product_owner_required' });
    expect(
      evaluateAdminAccess({ ...base, expectedProductOwnerToken: validToken })
    ).toMatchObject({ allowed: false, reason: 'role_tokens_must_be_distinct' });
    for (const runtimeSurface of [
      'vercel-preview',
      'vercel-production',
      'commerce-preview',
      'commerce-production',
      'commerce-unconfigured',
    ]) {
      expect(
        evaluateAdminAccess({ ...base, runtimeSurface }).allowed,
        runtimeSurface
      ).toBe(false);
    }
    expect(
...
describe('remote Product Owner identity boundary', () => {
  ...
  it('allows only the exact configured Product Owner on Vercel Preview or Production', () => {
    ...
    expect(
      evaluateRemoteProductOwnerAccess({ ...base, runtimeSurface: 'local' })
        .allowed
    ).toBe(false);
    expect(
      evaluateRemoteProductOwnerAccess({
    ...
    expect(
      evaluateRemoteProductOwnerAccess({ ...base, authenticatedUserId: null })
        .reason
    ).toBe('authenticated_session_required');
    expect(
      evaluateRemoteProductOwnerAccess({
        ...base,
        authenticatedUserId: 'user_reviewer123',
      }).reason
  ...
  it('keeps route protection server-side and limits Clerk middleware to admin resources', () => {
    ...
    const signInRoute = readFileSync(
      'app/admin/sign-in/[[...sign-in]]/page.js',
      'utf8'
    );
    const middleware = readFileSync('middleware.js', 'utf8');

    expect(route.indexOf('requireAdminAccess')).toBeLessThan(
      route.indexOf('loadAdminControlPlane')
    );
...
describe('admin operational projection', () => {
  const model = deriveAdminControlPlane({
    release,
    mediaManifest,
    run,
    registry,
  ...
  it('validates the readiness index and reviewed command/event foundations', () => {
    expect(ajv.validate(readinessSchema, readiness), ajv.errorsText()).toBe(
      true
    );
    expect(new Set(readiness.stages.map((stage) => stage.id)).size).toBe(
      readiness.stages.length
    );
    expect(readiness.stages).toHaveLength(13);
    expect(
      readiness.stages.filter((stage) =>
        ['succeeded', 'verified'].includes(stage.status)
      )
    ).toHaveLength(0);
    expect(
      readiness.stages.reduce(
        (counts, stage) => ({
          ...counts,
    ...
    expect(
      readiness.stages.find((stage) => stage.id === 'pod-mapping')
    ).toMatchObject({
    ...
    expect(ajv.validate(adminCommandSchema, command), ajv.errorsText()).toBe(
      true
    );
    ...
    expect(ajv.validate(operationalEventSchema, event), ajv.errorsText()).toBe(
      true
    );
  });
  ...
  it('reports canonical Staged truth and does not synthesize end-to-end readiness', () => {
    ...
    expect(
      model.blockers.find((blocker) => blocker.stageId === 'cart-checkout')
        ?.code
    ).toBe('CHECKOUT_AUTHORITY_NOT_RELEASE_BOUND');
    expect(model.release.bindings).toEqual(
  ...
  it('labels audit and lifecycle projections without inventing durable operations', () => {
    expect(
      adminSections.find((section) => section.id === 'audit')?.description
    ...
    expect(model.lifecycle.analytics.title).toBe(
      'No approved analytics ledger exists.'
    );
  ...
  it('projects a truthful empty command queue with every execution dependency unavailable', () => {
    ...
    expect(
      model.commands.rows.find((row) => row.id === 'policy')
    ).toMatchObject({
  ...
  it('keeps admin private, dynamic, read-only, and out of customer navigation', () => {
    ...
    expect(component).toContain(
      "section.id !== 'theme' || viewerRole === 'product_owner'"
    );
    expect(component).toContain(
      "section.id !== 'media-generation' || (viewerRole === 'product_owner' && mediaGenerationEnabled)"
    );
    expect(route).toContain(
      "['theme', 'media-generation'].includes(activeSection) ? 'product_owner' : null"
    );
    expect(route).toContain(
      "activeSection === 'media-generation' && !mediaGenerationEnabled"
    );
