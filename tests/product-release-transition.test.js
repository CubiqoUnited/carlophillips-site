...
describe('Product Release Record transitions', () => {
  ...
  it('denies tampered and cross-release candidate evidence bindings', () => {
    ...
    crossRelease.candidate.stagingEvidence.releaseId =
      'cp-other-release-2026-001';
    expect(
      blockerCodes(
        evaluateProductReleaseTransition({
          record: crossRelease,
  ...
  it('fingerprints equivalent evidence objects independently of key insertion order', () => {
    ...
    reordered.assets = manifest.assets.map((asset) =>
      Object.fromEntries(Object.entries(asset).reverse())
    );
    expect(fingerprintReleaseArtifact(reordered)).toBe(
  ...
  it('denies cross-candidate approval evidence even when its status says approved', () => {
    ...
    expect(decision.allowed).toBe(false);
    expect(blockerCodes(decision)).toContain(
      'PRODUCT_APPROVAL_BINDING_INVALID'
    );
  });
  ...
  it('allows Approved → Released only after active and rollback observations', () => {
    ...
    record.shopify.productionObservation =
      releasedEvidence.shopify.productionObservation;
    record.shopify.productionObservation.variantFingerprint =
      record.shopify.variantFingerprint;
    record.shopify.productionObservation.commerceFactsFingerprint =
      record.shopify.commerceFactsFingerprint;
    record.shopify.productionObservation.reviewedObservationFingerprint =
      record.shopify.observationFingerprint;
    record.rollback.verificationEvidence =
      releasedEvidence.rollback.verificationEvidence;
    const decision = evaluateProductReleaseTransition({
      record,
      manifest: createCompleteMediaManifest(),
      targetState: 'released',
