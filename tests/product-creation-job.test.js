...
describe('ProductCreationJob', () => {
  ...
  it('reproduces both durable jobs through the deterministic factory', () => {
    for (const job of [designerJob, trendJob]) {
      ...
      expect(computeCreationInputFingerprint(job)).toBe(job.idempotency.inputFingerprint);
    }
  });
  ...
  it('distinguishes on-demand and scheduled cadence without authorizing execution', () => {
    ...
    expect(trendJob.brief.researchPolicy.mayTriggerExternalResearch).toBe(false);
  ...
  it('rejects inconsistent freshness, copying authority, and missing binding brand rules', () => {
    ...
    expect(() => createCandidatePipelineRun(falselyFresh)).toThrow(/beyond its freshness limit/);

    const copyAuthorized = structuredClone(designerJob);
    copyAuthorized.brief.referenceUsePolicy.mayCopy = true;
    expect(() => createCandidatePipelineRun(copyAuthorized)).toThrow(/inspiration-only/);

    const unconstrained = structuredClone(designerJob);
    unconstrained.brief.brandConstraints.rules = [];
    expect(() => createCandidatePipelineRun(unconstrained)).toThrow(/binding brand rules/);

    const impossibleTimeline = structuredClone(designerJob);
    impossibleTimeline.brief.inputEvidence[0].provenance.retrievedAt = '2026-07-21T00:00:00Z';
    expect(() => createCandidatePipelineRun(impossibleTimeline)).toThrow(/invalid freshness window/);
  });
  ...
  it('suppresses retries and equivalent duplicate jobs without mutating the accepted registry', () => {
    ...
    equivalent.contractBindings.pipelineRunId = 'cp-hoodie-designer-equivalent-sim';
