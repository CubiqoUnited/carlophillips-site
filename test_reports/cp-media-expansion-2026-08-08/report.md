...
| On-body stills | Ten-shot MODA draft generated; six curated Preview candidates | MODA | Keep candidate-only; replace or re-review any frame with logo/garment drift |
| Multi-angle stills | Front, three-quarter, seated, side and digital back studies generated | MODA | Treat as campaign visualisations, not photographed spin-source truth |
...
| 360 spin | Missing; existing Spin Studio still requires a sequence/GLB | Compatible AI-spin experiment | Test a free existing-photo route and label the result as a digital POD visualisation |
| Interactive 3D/AR | Missing; Instant 3D iframe refuses connection and direct dashboard returns HTTP 400 | Compatible photo-to-GLB experiment | Generate one free draft from approved product imagery; inspect geometry/textures before headless integration |
...
- `public/products/signature-hoodie/candidates/moda/`: six curated MODA outputs covering full front, three-quarter, seated, side profile, digital back and product-alone flat lay. All remain Preview-only visualisations; the back view is explicitly unverified.
- `test_reports/cp-media-expansion-2026-08-08/moda-draft/`: all ten original MODA outputs. Shot 10 is quarantined because its neck label contains an invented mark; Shots 3, 4 and 7 are retained as unselected evidence.
...
- MODA official listing: https://apps.shopify.com/moda-ai. One Standard-resolution ten-shot job was run after Product Owner file upload; the action button stated `1 Credit - 10 images`. Ten originals were downloaded, no Shopify export was used, and no plan or paid credit was accepted.
- Instant 3D official listing: https://apps.shopify.com/instant-3d. Instant 3D is installed, but its embedded dashboard at `3dcloud.com.tr` refuses the Shopify iframe connection and direct access returns HTTP 400. No model, widget, credit use, or product-media mutation occurred.
- Spinr official listing: https://apps.shopify.com/spinr. It advertises three free AI 360 spins without a card, generated from existing product photos, with an MP4 and embed. It is a candidate POD bridge, not installed proof.
- 3Dify official listing: https://apps.shopify.com/3dify. It advertises three free AI 3D generations per month from single/multi-view product images with GLB/model-viewer output. It is a candidate replacement for broken Instant 3D, not installed proof.
...
The curated MODA assets plus the macro and still-derived motion studies render only when `environment === 'preview'` and the exact handle is `carlophillips-signature-hoodie`. Production and all other product handles omit the complete editorial-study section. Component tests require the complete MODA candidate path to remain absent from production and rejected Shot 10 to remain absent from the Preview.
